...
#define CORE_SMALL_VMA          2
    ...
    /* Align to page */
       ...
       if (set_gzip_header(cprm->file) < 0) {
       ...
       if (coredump_alloc_workspaces() < 0) {
       ...
       if (ret_comp_val < 0) {
		...
		unsigned long end;
...
#ifdef CONFIG_BINFMT_ELF_COMP
               if ((vma->vm_next) == NULL)     /* gzip Z_FINISH, coredump last step condition */
                       last_vma = 1;
               ...
               if (vma_size < ULTIMATE_COMP_BUF_SIZE)
               {
                       ...
                       //printk(KERN_ALERT "##### Coredump fail, goto end_coredump...\n");
                       //goto end_coredump;
               }
		...
		for (addr = vma->vm_start; addr < end; addr += PAGE_SIZE) {
			...
			if (page) {
				void *kaddr = kmap(page);
                       ...
                       ++user_page_cnt;
                       //++kernel_page_cnt;
                       memcpy((uncomp_src_buf+(vma_page_cnt * PAGE_SIZE)), kaddr, PAGE_SIZE);
			...
			if (stop)
               ...
               {
                       printk(KERN_ALERT "##### coredump logic Bug, buf_usage_cnt :%d, line : %d\n", buf_usage_cnt, __LINE__);
               }
               else if (buf_usage_cnt == 1)
               {
                       ...
                       switch (vma_type)
                       {
                               case CORE_SMALL_VMA:
                                       ...
                                       ret_comp_val = compress_coredump(cprm->file, uncomp_src_buf, compressed_buf,vma_size,&crc32_val, &z_finish);
