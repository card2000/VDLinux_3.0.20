...
/*
 *  tomoyo_same_restrict_privilege - Check for duplicated "struct tomoyo_restrict_privilege" entry.
 * 
...
static bool tomoyo_same_restrict_privilege(const struct tomoyo_acl_head *a,
                                   const struct tomoyo_acl_head *b)
{
        const struct tomoyo_restrict_privilege *p1 = container_of(a, typeof(*p1),
                                                          head);
        const struct tomoyo_restrict_privilege *p2 = container_of(b, typeof(*p2),
                                                          head);

        return p1->is_last_name == p2->is_last_name &&
...
/*
 *  tomoyo_write_restrict_privilege - Write "struct tomoyo_restrict_privilege" list.
 *
 *  @param: Pointer to "struct tomoyo_acl_param".
 *
 *  Returns 0 on success, negative value otherwise.
...
int tomoyo_write_restrict_privilege(struct tomoyo_acl_param *param)
{
        struct tomoyo_restrict_privilege trp = { };
        int error = param->is_delete? -ENOENT: -ENOMEM;
        char *domainname = tomoyo_read_token(param);
        char *uid = tomoyo_read_token(param);
        char *gid = tomoyo_read_token(param);
        u8 type;
        unsigned long v;
        ...
        if (domainname)
        {
                if (!tomoyo_correct_domain(domainname))
                {
                        if (!tomoyo_correct_path (domainname))
                                goto out;
                        trp.is_last_name = true;
                }

                trp.domainname = tomoyo_get_name(domainname);
                if (!trp.domainname)
                        goto out;

                type = tomoyo_parse_ulong(&v, &uid);
                if (type != TOMOYO_VALUE_TYPE_DECIMAL)
		{
			tomoyo_put_name(trp.domainname);
                        goto out;
		}
                trp.uid = v;

                type = tomoyo_parse_ulong(&v, &gid);
                if (type != TOMOYO_VALUE_TYPE_DECIMAL)
		{
			tomoyo_put_name(trp.domainname);
                        goto out;
        ...
        param->list = &param->ns->policy_list[TOMOYO_ID_RESTRICT_PRIVILEGE];

        error = tomoyo_update_policy(&trp.head, sizeof(trp), param, tomoyo_same_restrict_privilege);
					   ...
					   const struct tomoyo_acl_info *))
{
	...
	switch (r->param_type) {
		case TOMOYO_TYPE_PATH_ACL:
			list = &domain->acl_info_list[tomoyo_p2operation[r->param.path.operation]];
			break;
		case TOMOYO_TYPE_PATH2_ACL:
			list = &domain->acl_info_list[tomoyo_pp2operation[r->param.path2.operation]];
			break;
		case TOMOYO_TYPE_PATH_NUMBER_ACL:
			list = &domain->acl_info_list[tomoyo_pn2operation[r->param.path_number.operation]];
			break;
		case TOMOYO_TYPE_MKDEV_ACL:
			list = &domain->acl_info_list[tomoyo_pnnn2operation[r->param.mkdev.operation]];
			break;
		case TOMOYO_TYPE_MOUNT_ACL:
			list = &domain->acl_info_list[TOMOYO_MOUNT];
			break;
		case TOMOYO_TYPE_UNIX_ACL:
			list = &domain->acl_info_list[TOMOYO_NETWORK_UNIX];
			break;
		case TOMOYO_TYPE_INET_ACL:
			list = &domain->acl_info_list[TOMOYO_NETWORK_INET];
			break;
		case TOMOYO_TYPE_ENV_ACL:
			list = &domain->acl_info_list[TOMOYO_ENVIRON];
			break;
		case TOMOYO_TYPE_MANUAL_TASK_ACL:
			list = &domain->acl_info_list[TOMOYO_MANUAL_TASK_ACL];
			break;
		default:
			BUG_ON(list == NULL);
			return;
						...
						const bool transit)
{
	...
	if (!entry) {
		...
		if (entry) {
			for (i = 0; i < TOMOYO_MAX_OPERATIONS; i++)
				INIT_LIST_HEAD(&entry->acl_info_list[i]);
...
int tomoyo_find_next_domain(struct linux_binprm *bprm)
{
	...
	if (tomoyo_enable){
		...
		char *namestr = NULL;
		unsigned int buf_len = strlen(original_name) + 1;
		namestr = kmalloc(buf_len, GFP_NOFS);
		if (namestr)
		        strncpy(namestr, original_name, buf_len);
		exename.name = namestr;
	}
#endif
	if (!exename.name)
		goto out;
