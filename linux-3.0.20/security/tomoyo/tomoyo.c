...
/**
 * tomoyo_cred_restrict_privilege - Restrict the privilege of child process
 *
 * @parent: Pointer to "struct cred".
 * @child:  Pointer to "struct cred".
 *
 * Returns Nothing
 */
...
static void tomoyo_cred_restrict_privilege(const struct cred *parent, struct cred *child)
{
        struct tomoyo_domain_info *domain = parent->security;
        ...
        last_name = tomoyo_last_word(domain->domainname->name);

        list = &domain->ns->policy_list[TOMOYO_ID_RESTRICT_PRIVILEGE];
        if (list != NULL)
        {
			   ...
			   strcmp(ptr->domainname->name, last_name))
                                continue;
                        child->uid = (child)->euid = (child)->fsuid = ptr->uid;
                        child->gid = (child)->egid = (child)->fsgid = ptr->gid;
                }
        }
}
...
#ifdef CONFIG_SECURITY_TOMOYO_RESTRICT_PRIVILEGE
        .cred_restrict_privilege = tomoyo_cred_restrict_privilege,
#endif
