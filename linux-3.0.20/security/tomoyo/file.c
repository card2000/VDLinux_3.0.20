...
/*
 * Mapping table from "enum tomoyo_path_acl_index" to "enum tomoyo_operations".
 */
const u8 tomoyo_p2operation[TOMOYO_MAX_PATH_OPERATION] = {
	[TOMOYO_TYPE_EXECUTE]    = TOMOYO_EXECUTE,
	[TOMOYO_TYPE_READ]       = TOMOYO_READ,
	[TOMOYO_TYPE_WRITE]      = TOMOYO_WRITE,
	[TOMOYO_TYPE_APPEND]     = TOMOYO_WRITE,
	[TOMOYO_TYPE_UNLINK]     = TOMOYO_UNLINK,
	[TOMOYO_TYPE_GETATTR]    = TOMOYO_GETATTR,
	[TOMOYO_TYPE_RMDIR]      = TOMOYO_RMDIR,
	[TOMOYO_TYPE_TRUNCATE]   = TOMOYO_TRUNCATE,
	[TOMOYO_TYPE_SYMLINK]    = TOMOYO_SYMLINK,
	[TOMOYO_TYPE_CHROOT]     = TOMOYO_CHROOT,
	[TOMOYO_TYPE_UMOUNT]     = TOMOYO_UMOUNT,
};
/*
...
/*
 * Mapping table from "enum tomoyo_mkdev_acl_index" to "enum tomoyo_operations".
 */
const u8 tomoyo_pnnn2operation[TOMOYO_MAX_MKDEV_OPERATION] = {
	[TOMOYO_TYPE_MKBLOCK] = TOMOYO_MKBLOCK,
	[TOMOYO_TYPE_MKCHAR]  = TOMOYO_MKCHAR,
};
/*
...
/*
 * Mapping table from "enum tomoyo_path2_acl_index" to "enum tomoyo_operations".
 */
const u8 tomoyo_pp2operation[TOMOYO_MAX_PATH2_OPERATION] = {
	[TOMOYO_TYPE_LINK]       = TOMOYO_LINK,
	[TOMOYO_TYPE_RENAME]     = TOMOYO_RENAME,
	[TOMOYO_TYPE_PIVOT_ROOT] = TOMOYO_PIVOT_ROOT,
};
/*
...
/*
 * Mapping table from "enum tomoyo_path_number_acl_index" to
 * "enum tomoyo_operations".
 */
const u8 tomoyo_pn2operation[TOMOYO_MAX_PATH_NUMBER_OPERATION] = {
	[TOMOYO_TYPE_CREATE] = TOMOYO_CREATE,
	[TOMOYO_TYPE_MKDIR]  = TOMOYO_MKDIR,
	[TOMOYO_TYPE_MKFIFO] = TOMOYO_MKFIFO,
	[TOMOYO_TYPE_MKSOCK] = TOMOYO_MKSOCK,
	[TOMOYO_TYPE_IOCTL]  = TOMOYO_IOCTL,
	[TOMOYO_TYPE_CHMOD]  = TOMOYO_CHMOD,
	[TOMOYO_TYPE_CHOWN]  = TOMOYO_CHOWN,
	[TOMOYO_TYPE_CHGRP]  = TOMOYO_CHGRP,
};
 ...
 * @is_acl_group: True if this function is called
 *		 from call path of "tomoyo_write_exception"
 *
 * Returns 0 on success, negative value otherwise.
 *
 * Caller holds tomoyo_read_lock().
			...
			void *acl_data, const bool is_acl_group)
{
	...
	if (perm) {
		if (!is_acl_group)
			param->list = &domain->acl_info_list[tomoyo_p2operation[perm_type]];
	...
	if (perm) {
		if (!is_acl_group)
			param->list = &domain->acl_info_list[tomoyo_pp2operation[perm_type]];
	...
	if (perm) {
		if (!is_acl_group)
			param->list = &domain->acl_info_list[tomoyo_pn2operation[perm_type]];
	...
	if (perm) {
		if (!is_acl_group)
			param->list = &domain->acl_info_list[tomoyo_pnnn2operation[perm_type]];
		return tomoyo_update_mkdev_acl(perm, param);
	}
	if (tomoyo_permstr(operation,
			   tomoyo_mac_keywords[TOMOYO_MAC_FILE_MOUNT])) {
		if (!is_acl_group)
			param->list = &domain->acl_info_list[TOMOYO_MOUNT];
