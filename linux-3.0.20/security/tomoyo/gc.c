...
/**
 * tomoyo_del_restrict_privilege - Delete members in "struct tomoyo_restrict_privilege".
 *
 * @element: Pointer to "struct list_head".
 *
 * Returns nothing.
...
static inline void tomoyo_del_restrict_privilege(struct list_head *element)
{
...
static inline void tomoyo_del_domain(struct list_head *element)
{
	...
	for (i=0; i<TOMOYO_MAX_OPERATIONS; i++) {
		list_for_each_entry_safe(acl, tmp, &domain->acl_info_list[i], list) {
			     ...
			     struct list_head *element)
{
	...
	case TOMOYO_ID_AGGREGATOR:
        ...
        case TOMOYO_ID_RESTRICT_PRIVILEGE:
                tomoyo_del_restrict_privilege(element);
                break;
#endif
...
static void tomoyo_collect_entry(void)
{
	...
	mutex_lock(&tomoyo_policy_lock);
	{
		...
		list_for_each_entry_safe(domain, tmp, &tomoyo_domain_list,
					 list) {
			for(i=0; i<TOMOYO_MAX_OPERATIONS; i++)
				tomoyo_collect_acl(&domain->acl_info_list[i]);
