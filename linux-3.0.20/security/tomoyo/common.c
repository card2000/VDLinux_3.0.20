...
static const char *hooks_list[MAX_SECURITY_HOOK_ID][2] = {
       {"unlink", "security_path_unlink"},
       {"mkdir", "security_path_mkdir"},
       {"rmdir", "security_path_rmdir"},
       {"mknod", "security_path_mknod"},
       {"truncate", "security_path_truncate"},
       {"symlink", "security_path_symlink"},
       {"link", "security_path_link"},
       {"rename", "security_path_rename"},
       {"chmod", "security_path_chmod"},
       {"chown", "security_path_chown"},
       {"chroot", "security_path_chroot"},
       {"mount", "security_sb_mount"},
       {"umount", "security_sb_umount"},
       {"pivotroot", "security_sb_pivotroot"},
       {"exec", "security_bprm_set_creds"},
       {"exec", "security_bprm_check"},
       {"fork", "security_cred_alloc_blank"},
       {"fork", "security_cred_free"},
       {"fork", "security_prepare_creds"},
       {"fork", "security_transfer_creds"},
       {"open", "security_dentry_open"},
       {"fcntl", "security_file_fcntl"},
       {"ioctl", "security_file_ioctl"},
       {"Permission check","security_file_permission"},
       {"msgsnd", "security_msg_msg_alloc"},
       {"prctl", "security_task_prctl"},
       {"brk","security_vm_enough_memory"},
       {"getattr", "security_inode_getattr"},
       {"path lookup", "security_inode_exec_permission"},
       {"execv", "security_vm_enough_memory_mm"},
       {"semctl", "security_sem_semctl"},
       {"ipc permission check", "security_ipc_permission"},
       {"capability checking", "security_capable"},
       {"inode permission check", "security_inode_permission"},
       {"link and symlink", "security_inode_follow_link"}
};
...
#ifdef CONFIG_SECURITY_TOMOYO_POLICY_READ
...
#endif /* CONFIG_SECURITY_TOMOYO_POLICY_READ */
...
#endif /* CONFIG_SECURITY_TOMOYO_MANAGER */

#if defined (CONFIG_SECURITY_TOMOYO_POLICY_UPDATE) || defined (CONFIG_SECURITY_TOMOYO_POLICY_READ) || defined (CONFIG_SECURITY_TOMOYO_LOG_READER)

/**
 * tomoyo_policy_controller - Check whether the current process can
 *				perform policy related operations
 *
 * Returns true if the current process is permitted to perform policy
 * operations via /sys/kernel/security/tomoyo/ interface.
 *
 */

static bool tomoyo_policy_controller(int type)
{
        const char *exe;
	...
	switch (type) {
#ifdef CONFIG_SECURITY_TOMOYO_POLICY_UPDATE
	case TOMOYO_POLICY_WRITER:
			policy_controller = CONFIG_SECURITY_TOMOYO_POLICY_WRITER_NAME;
			break;
#endif

#ifdef CONFIG_SECURITY_TOMOYO_POLICY_READ
	case TOMOYO_POLICY_READER:
			policy_controller = CONFIG_SECURITY_TOMOYO_POLICY_READER_NAME;
...
#ifdef CONFIG_SECURITY_TOMOYO_LOG_READER
	case TOMOYO_LOG_READER:
			policy_controller = CONFIG_SECURITY_TOMOYO_LOG_READER_NAME;
			break;
#endif
	}

	if(!policy_controller) {
		printk(KERN_WARNING "No process can perform requested operation. \n");
	        return false;
	}
        ...
        if (strcmp(exe, policy_controller)) {
                printk(KERN_WARNING "%s is not permitted to "
 ...
 * @acl_data: Void pointer.
 * @is_acl_group: True if this function is in call path from
...
static int tomoyo_write_task(struct tomoyo_acl_param *param,
				void *acl_data, const bool is_acl_group)
{
	struct tomoyo_domain_info *domain;
	int error = -EINVAL;
        if (!is_acl_group){
                domain = (struct tomoyo_domain_info *) acl_data;
		param->list = &domain->acl_info_list[TOMOYO_MANUAL_TASK_ACL];
	} else {
		param->list = (struct list_head *) acl_data;
	}
 ...
 * @is_delete:   True if it is a delete request.
 * @is_acl_group:True if it is called from tomoyo_write_exception.
 *
 * Returns 0 on success, negative value otherwise.
 *
 * Caller holds tomoyo_read_lock().
...
static int tomoyo_write_domain2(struct tomoyo_policy_namespace *ns,
				void *acl_data, char *data,
				const bool is_delete, const bool is_acl_group)
{
	struct tomoyo_acl_param param = {
		.ns = ns,
		.data = data,
	...
	static const struct {
		const char *keyword;
		int (*write) (struct tomoyo_acl_param *, void *, const bool);
	} tomoyo_callback[5] = {
	...
	for (i = 0; i < ARRAY_SIZE(tomoyo_callback); i++) {
		if (!tomoyo_str_starts(&param.data,
			...
			continue;
		return tomoyo_callback[i].write(&param, acl_data, is_acl_group);
	}
	return -EINVAL;
}
...
static int tomoyo_write_domain(struct tomoyo_io_buffer *head)
{
	...
	return tomoyo_write_domain2(ns, domain, data,
				    is_delete, false);
}
...
static void tomoyo_read_domain(struct tomoyo_io_buffer *head)
{
	...
	list_for_each_cookie(head->r.domain, &tomoyo_domain_list) {
		...
		case 0:
			...
			/* fall through */
		case 1:
			while ( head->r.list_pos < TOMOYO_MAX_OPERATIONS)
			{
				if (tomoyo_read_domain2(head, &domain->acl_info_list[head->r.list_pos]))
					head->r.list_pos++;
				else 
					return;
			}
...
static int tomoyo_write_exception(struct tomoyo_io_buffer *head)
{
	...
	if (tomoyo_str_starts(&param.data, "aggregator "))
        ...
        if (tomoyo_str_starts(&param.data, "restrict_privilege "))
                return tomoyo_write_restrict_privilege(&param);
#endif
	...
	list_for_each_cookie(head->r.acl, list) {
		...
		case TOMOYO_ID_AGGREGATOR:
			...
			break;
                ...
                case TOMOYO_ID_RESTRICT_PRIVILEGE:
                        {
                                struct tomoyo_restrict_privilege *ptr =
                                        container_of(acl, typeof(*ptr), head);
                                char buffer[256];

                                tomoyo_print_namespace(head);
                                tomoyo_set_string(head, "restrict_privilege ");
                                tomoyo_set_string(head,
                                                  ptr->domainname->name);
                                tomoyo_set_space(head);
                                snprintf(buffer, 256, "%u %u", ptr->uid, ptr->gid);
                                tomoyo_set_string(head, buffer);
                        }
                        break;
#endif
...
static void tomoyo_add_entry(struct tomoyo_domain_info *domain, char *header)
{
	...
	*cp++ = '\0';
	...
	if (!tomoyo_write_domain2(domain->ns, domain, buffer,
				  false, false))
...
/**
 * tomoyo_write_enable - This function will set the tomoyo_enable 
 *                       flag. This flag will be used to find out 
 *                       whether tomoyo hook should be called from 
 *                       LSM hook.
 *  @head: Pointer to "struct tomoyo_io_buffer".
 * 
 *  Returns nothing.
 */
static int tomoyo_write_enable(struct tomoyo_io_buffer *head)
{
        char *data = head->write_buf;

        sscanf(data, "%u", &tomoyo_enable);
	return 0;
}


/**
 * tomoyo_read_enable - This function will set the tomoyo_enable
 *                      flag.This flag will be used to find out 
 *                      whether tomoyo hook should be called from 
 *                      LSM hook.
 *  @head: Pointer to "struct tomoyo_io_buffer".
 ...
 *  Returns nothing.
 */
static void tomoyo_read_enable(struct tomoyo_io_buffer *head)
{
        if(head->r.eof)
                return;
        tomoyo_io_printf(head, "%u", tomoyo_enable);
        tomoyo_set_lf(head);
        head->r.eof = true;
}
#endif /* CONFIG_SECURITY_TOMOYO_DISABLE */

#ifdef CONFIG_SECURITY_DEBUG
/**
 * tomoyo_write_check_performance - This function will set the  flag
 *				    This flag will be used to find out 
 *				    if we want to get run time of tomoyo 
 *				    hook.
 * @head: Pointer to "struct tomoyo_io_buffer".
 *
 * Returns nothing.
 */
static int tomoyo_write_check_performance(struct tomoyo_io_buffer *head)
{
	char *data = head->write_buf;

	sscanf(data, "%u", &check_performance);
	return 0;
}

/**
 * tomoyo_read_check_performance - This function will set the  flag
 *                                  This flag will be used to find out 
 *                                  if we want to get run time of tomoyo 
 *                                  hook.
 * @head: Pointer to "struct tomoyo_io_buffer".
 ...
 * Returns nothing.
 *         */
static void tomoyo_read_check_performance(struct tomoyo_io_buffer *head)
{
	if(head->r.eof)
		return;
	tomoyo_io_printf(head, "%u", check_performance);
	tomoyo_set_lf(head);
	head->r.eof = true;
...
/**
 * tomoyo_read_hooks_calltime - This function will 
 *				measure time taken
 *				by all TOMOYO hooks.
 * @head: Pointer to "struct tomoyo_io_buffer".
 *
 * Returns nothing
 */
static void tomoyo_read_hooks_calltime(struct tomoyo_io_buffer *head)
{
	int i = 0;
	unsigned long total_counter = 0, total_time = 0;
	unsigned long flags;
	if(head->r.eof)
		return;
	tomoyo_io_printf(head, "\n\tSystem Call \t\t  LSM Hook \t\tCounter     Min time    Max Time   Avg time    Total Time\n");
	spin_lock_irqsave(&security_debug, flags);
	for (i = 0; i < MAX_SECURITY_HOOK_ID; i++) {
		tomoyo_io_printf(head, "%22s %31s  %7lu  %8lu  %12lu  %10lu  %12lu", hooks_list[i][0], hooks_list[i][1],lsm_call_time[i].counter, lsm_call_time[i].min_time,lsm_call_time[i].max_time, lsm_call_time[i].avg_time,lsm_call_time[i].total_time);
		tomoyo_set_lf(head);
	}
	tomoyo_io_printf(head, "=============================================================================================================================\n");
	for (i = 0; i < MAX_SECURITY_HOOK_ID; i++) {
		total_counter += lsm_call_time[i].counter;
		total_time += lsm_call_time[i].total_time;
	}
	tomoyo_io_printf(head, "\n\t\t\t\t Total \t\t\t %7lu         -          -            -   %12lu", total_counter, total_time);  
	tomoyo_set_lf(head);
...
#ifdef CONFIG_SECURITY_TOMOYO_DISABLE
	case TOMOYO_ENABLE:
	/* /sys/kernel/security/tomoyo/tomoyo_enable */
	head->write = tomoyo_write_enable;
	head->read = tomoyo_read_enable;
	break;
#endif

#ifdef CONFIG_SECURITY_DEBUG
	case TOMOYO_HOOKS_CALLTIME_CONTROL:
		/* /sys/kernel/security/tomoyo/check_performance */
		head->read = tomoyo_read_check_performance;
		head->write = tomoyo_write_check_performance;
		break;

	case TOMOYO_HOOKS_CALLTIME:
		/* /sys/kernel/security/tomoyo/hooks_calltime */
		head->read = tomoyo_read_hooks_calltime;
		break;
#endif /* CONFIG_SECURITY_DEBUG */
...
#ifdef CONFIG_SECURITY_TOMOYO_POLICY_READ
			if(head->read != tomoyo_read_log) {
				if( !tomoyo_policy_controller(TOMOYO_POLICY_READER))
					return -EPERM;
			}
#endif
#ifdef CONFIG_SECURITY_TOMOYO_LOG_READER
                        if (head->read == tomoyo_read_log) {
                                if (!tomoyo_policy_controller(TOMOYO_LOG_READER))
                                        return -EPERM;
                        }
#endif /* CONFIG_SECURITY_TOMOYO_LOG_READER */
			head->read(head);
		...
		default:
			...
			if (!tomoyo_manager()) {
				...
				goto out;
                        ...
                        if (!tomoyo_policy_controller(TOMOYO_POLICY_WRITER)) {
                                error = -EPERM;
                                goto out;
                        }
#endif

#if !defined (CONFIG_SECURITY_TOMOYO_MANAGER) && !defined (CONFIG_SECURITY_TOMOYO_POLICY_UPDATE)
			break;
#endif
		}
