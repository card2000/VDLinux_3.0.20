...
#define LINUSTOREIII_VERSION_STRING      "LinuStoreIII_1.2.0_b039"
...
/* FSR supports max 36 BML partitions and max 16 STL partitions */
#define	    MAX_STL_PARTITIONS			(PARTITION_ID_FILESYSTEM15 - PARTITION_ID_FILESYSTEM0 + 1)
#endif

/**
 * Below portion of the file is shared by user and kernel space
 */
...
/*
 * ++++ Volumes, Partitions and Minor number policy ++++
 * Support for 36 FSR partitions per Volume(Max 2 Volumes).
 * Vol 0 - Minors 0 to 36(37 to 63 unused)
 *       - Minor 0 represents Whole Device(Chip0)
 *       - Minors 1 to 36 represent 36 FSR partitions for Vol0
 *
 * Vol 1 - Minors 64 to 100(101 to 127 unused)
 *       - Minor 64 represents Whole Device(Chip1)
 *       - Minors 65 to 100 represent 36 FSR partitions for Vol1

*/
#define MASK(x)						((1U << (x)) -1)	///< partition bit mask			
#define PARTITION_BITS				6					///< partition bit
#define PARTITION_MASK				MASK(PARTITION_BITS) ///< partition bit mask macro
#define MAX_FLASH_PARTITIONS		36					///< calculate number of partitions
