...
extern void sdp1207fpga_init(void);
extern void sdp1207fpga_iomap_init(void);
...
static void __init fpga_foxb_map_io(void)
{
	//initialize iomap of special function register address
	sdp1207fpga_iomap_init();
}

static void __init fpga_foxb_init(void)
{
	sdp1207fpga_init();
}

static void __init 
fpga_foxb_fixup(struct machine_desc *mdesc, struct tag *tags, char ** cmdline, struct meminfo *meminfo)
{
	int nbank = 0;
...
MACHINE_START(SDP1207FPGA_FOXB, "Samsung SDP1207 FPGA board")
	.boot_params  = (PHYS_OFFSET + 0x100),
	.map_io		= fpga_foxb_map_io,
	.init_irq	= sdp_gic_init_irq,
	.timer		= &sdp_timer,
	.init_machine	= fpga_foxb_init,
	.fixup		= fpga_foxb_fixup,
MACHINE_END
