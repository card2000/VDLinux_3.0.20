...
//	AHCI_PCI_BAR		= 5,
	...
	/* command list (AHCI 4.2.2) entry DW2: this should be aligned with 128(80h) */
	AHCI_CMD_TBL_SZ		= AHCI_CMD_TBL_HDR_SZ + ((AHCI_MAX_SG + 7) & (~7)) * 16,
...
static struct scsi_host_template ahci_sht0 = {
...
static struct scsi_host_template ahci_sht1 = {
	ATA_NCQ_SHT(DRV_NAME),
	.can_queue		= AHCI_MAX_CMDS - 1,
...
extern int debug_lba  ;  //printk LBA address

static void ahci_print_DebugInfo(struct ata_queued_cmd *qc, u32 read_size, u32 read_count )
{
        static u32 prev_nextlba=0;
        ...
        switch ( cmnd )
        {
                ...
                case READ_12:
                case WRITE_12:
                        memcpy(astring, ( lba != prev_nextlba )?"\e[41m":"\e[44m", sizeof("\e[41m"));
                           if(debug_lba == 1 || debug_lba == 2)
                                printk("%sSetup.orig.command(0x%x:0x%x), LBA(0x%x), NOB(0x%x), Next(0x%x), size 0x%x, count %d\e[0m\n", astring,  qc->tf.command, cmnd, lba, nob, nextlba, read_size, read_count);
                        /* Keep data */
                        prev_nextlba = nextlba;
                        break;
                default:
                           if(debug_lba==2)
                                printk("%sSetup.orig.command(0x%x:0x%x), LBA(0x%x), NOB(0x%x), Next(0x%x), size 0x%x, count %d\e[0m\n", astring,  qc->tf.command, cmnd, lba, nob, nextlba, read_size, read_count);
...
#ifdef CONFIG_KDEBUGD
                uiReadSize += cpu_to_le32(sg_len);
                uiReadCount++;
#endif
	}

#ifdef CONFIG_KDEBUGD
        if(debug_lba)  //print DebugInfo  if debug_lba is enabled((19) COUNTER MONITOR: Disk Usage)
                ahci_print_DebugInfo(qc, uiReadSize, uiReadCount);
#endif
...
static irqreturn_t ahci_interrupt(int irq, void *dev_instance)
{
	...
	udelay(2);//120918 add interrupt delay
...
static int sata_ahci_probe(struct platform_device *pdev)
{
	...
	if (!strcmp(pdev->name, "ahci-sata0")) {
		ahci_sht = &ahci_sht0;
	} else {
		ahci_sht = &ahci_sht1;
	}
	dev_info(&pdev->dev, "sg entries = %d\n", ahci_sht->sg_tablesize);
	...
	rg=readl(hpriv->iomap + 0xbc);	  //OOBR 
	rg=rg|0x80000000;
	writel(rg, hpriv->iomap + 0xbc);

	rg=readl(hpriv->iomap + 0xbc);
	rg=rg|0x02060b14;
	writel(rg, hpriv->iomap + 0xbc);
	...
	rg=readl(hpriv->iomap + HOST_CAP);	//CAP
	writel(rg, hpriv->iomap + HOST_CAP);

	rg=readl(hpriv->iomap +0x0C );		//PI
	rg=rg|0x1;
	writel(rg, hpriv->iomap + 0x0C);

	rg=readl(hpriv->iomap + 0x118);	//P0CMD
	rg=rg &0xffc3ffff;
	writel(rg, hpriv->iomap + 0x118);

	rg=readl(hpriv->iomap + 0x118);
	rg=rg|0x10;
	writel(rg, hpriv->iomap + 0x118);
...
static void ahci_sata_phy0_init(void)
{
	...
	regval=ahci_get_reg(0x100908f4);
	regval=regval&0xfffCffff; 
	ahci_set_reg(0x100908f4, regval);

	udelay(1);

	regval=ahci_get_reg(0x100A0004);
	regval=regval|0x00010000; //Set MPLL_CK_OFF to 1'b1.MPLL_PRESCALE[1:0], MPLL_NCY[4:0] (100Mhz)
	ahci_set_reg(0x100A0004, regval);

	regval=ahci_get_reg(0x100A0004);
	regval=regval|0x00000206; //Set MPLL_CK_OFF to 1'b1.MPLL_PRESCALE[1:0], MPLL_NCY[4:0] (100Mhz)
	ahci_set_reg(0x100A0004, regval);

	regval=ahci_get_reg(0x100A0000);
	regval=regval|0x00000020; //Set MPLL_NCY5[1:0] (100Mhz)
	ahci_set_reg(0x100A0000, regval);

	regval=ahci_get_reg(0x100A0004);
	regval=regval&~(0x00010000); //Set MPLL_CK_OFF to 1'b0.
	ahci_set_reg(0x100A0004, regval);
	...
	//link reset(ARESETX_I)
	regval=ahci_get_reg(0x100908f4);
	regval=regval|0x00010000; 
	ahci_set_reg(0x100908f4, regval);
...
//	ahci_set_reg(0x30040000, 0x6726ff80);//HWinit
...
static void ahci_sata_phy1_init(void)
{
	...
	regval=ahci_get_reg(0x100908f4);
	regval=regval&0xfff3ffff; 
	ahci_set_reg(0x100908f4, regval);
	//100us
	udelay(1);

	regval=ahci_get_reg(0x100A000c);
	regval=regval|0x00010000; //Set MPLL_CK_OFF to 1'b1 MPLL_PRESCALE[1:0], MPLL_NCY[4:0] (100Mhz)
	ahci_set_reg(0x100A000c, regval);

	regval=ahci_get_reg(0x100A000c);
	regval=regval|0x00000206; //Set MPLL_CK_OFF to 1'b1 MPLL_PRESCALE[1:0], MPLL_NCY[4:0] (100Mhz)
	ahci_set_reg(0x100A000c, regval);

	regval=ahci_get_reg(0x100A0008);
	regval=regval|0x00000020; //Set MPLL_NCY5[1:0] (100Mhz)
	ahci_set_reg(0x100A0008, regval);
...
//	ahci_set_reg(0x30050000, 0x6726ff80);//HWinit
}
