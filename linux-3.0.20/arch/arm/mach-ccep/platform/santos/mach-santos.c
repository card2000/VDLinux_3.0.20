...
extern void sdp1103_init(void);
extern void sdp1103_iomap_init(void);
...
static void __init santos_map_io(void)
{
	//initialize iomap of special function register address
	sdp1103_iomap_init();
}

static void __init santos_init(void)
{
	sdp1103_init();
}

static void __init 
santos_fixup(struct machine_desc *mdesc, struct tag *tags, char ** cmdline, struct meminfo *meminfo)
{
	int nbank = 0;
...
MACHINE_START(SDP1103_SANTOS, "Samsung SDP1103 Evaluation board")
	.phys_io  = PA_IO_BASE0,
	.io_pg_offst = VA_IO_BASE0,
	.boot_params  = (PHYS_OFFSET + 0x100),
	.map_io		= santos_map_io,
	.init_irq	= sdp_init_irq,
	.timer		= &sdp_timer,
	.init_machine	= santos_init,
	.fixup		= santos_fixup,
MACHINE_END
