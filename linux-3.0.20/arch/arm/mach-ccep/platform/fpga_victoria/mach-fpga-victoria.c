...
extern void sdp1105fpga_init(void);
extern void sdp1105fpga_iomap_init(void);
...
static void __init fpga_victoria_map_io(void)
{
	//initialize iomap of special function register address
	sdp1105fpga_iomap_init();
}

static void __init fpga_victoria_init(void)
{
	sdp1105fpga_init();
}

static void __init 
fpga_victoria_fixup(struct machine_desc *mdesc, struct tag *tags, char ** cmdline, struct meminfo *meminfo)
{
	int nbank = 0;
	...
	printk("Board Memory : %dMB(%dMB+%dMB+%dMB+%dMB), Kernel Memory : %dMB(%dMB+%dMB+%dMB+%MB)\n"
...
MACHINE_START(SDP1105FPGA_VICTORIA, "Samsung SDP1105 FPGA board")
	.phys_io  = PA_IO_BASE0,
	.io_pg_offst = VA_IO_BASE0,
	.boot_params  = (PHYS_OFFSET + 0x100),
	.map_io		= fpga_victoria_map_io,
	.init_irq	= sdp_gic_init_irq,
	.timer		= &sdp_timer,
	.init_machine	= fpga_victoria_init,
	.fixup		= fpga_victoria_fixup,
MACHINE_END
