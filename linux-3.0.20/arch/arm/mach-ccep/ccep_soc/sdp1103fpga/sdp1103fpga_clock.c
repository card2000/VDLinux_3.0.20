/*
 * linux/arch/arm/mach-sdp/sdp1003fpga_clock.c
 *
 * Copyright (C) 2010 Samsung Electronics.co
 * Author : sh.chon@samsung.com 	06/21/2011
 *
 */
...
static int sdp1103fpga_init_clock(void);
...
unsigned long sdp1103fpga_get_clock (char mode)
{
// 20080524 by tukho.kim 
// apply HRTIMER 
	if(!fclk.rate) sdp1103fpga_init_clock();
...
static int __init sdp1103fpga_init_clock(void)
{
...
int __init clk_init(void)
{
	...
	printk(KERN_INFO "sdp1103fpga clock control (c) 2011 Samsung Electronics\n");
	sdp1103fpga_init_clock();

	for(i= 0; i< ARRAY_SIZE(init_clocks);i++) {
		clk = init_clocks[i];
		clk_register(clk);
		printk (KERN_INFO "SDP1103fpga %s Clock: %d.%03dMhz\n",
				clk->name,
