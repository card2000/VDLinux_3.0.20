/*
 * linux/arch/arm/mach-sdp/sdp1106fpga_clock.c
 *
...
static int sdp1106fpga_init_clock(void);
...
unsigned long sdp1106fpga_get_clock (char mode)
{
// 20080524 by tukho.kim 
// apply HRTIMER 
	if(!fclk.rate) sdp1106fpga_init_clock();
...
static int __init sdp1106fpga_init_clock(void)
{
...
int __init clk_init(void)
{
	...
	printk(KERN_INFO "sdp1106fpga clock control (c) 2011 Samsung Electronics\n");
	sdp1106fpga_init_clock();

	for(i= 0; i< ARRAY_SIZE(init_clocks);i++) {
		clk = init_clocks[i];
		clk_register(clk);
		printk (KERN_INFO "sdp1106fpga %s Clock: %d.%03dMhz\n",
				clk->name,
