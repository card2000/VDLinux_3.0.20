/*
 * sdp1004_misc.c
 * sdp1004 miscellaneous (ext interrupt, pad mux, and pin attribute)
 *
...
/*********************************************************/
#define A_SUBINT_STAT		(VA_IO_BASE0 + 0x00090F94)				// INT 31 sub	0 ~ 2->3~7 invaild extint
...
#define R_SUBINT_STAT		(*(volatile u32*)(A_SUBINT_STAT))	// INT 31 sub	0 ~ 2
#define NR_EXT_INTR		3
...
int sdp_extint_fp_register(int n_extirq, int attr, void (*fp)(void*), void* args)
{
	...
	if(sdp_eint[n_extirq].fp) {
		printk(KERN_ERR"[%s] %d Ext slot not empty\n", EXT_INTR_NAME, n_extirq);
		retval = -1;
		goto __out_register;		
	}
...
static void __init sdp_extint_preinit(void)
{
	...
	R_EXT_INTR_SEL = 0xFF; 	// all disable
...
static int __init sdp_extint_init(void)
{
	...
	retval = request_irq(31, sdp_eint_isr, IRQF_DISABLED, "sdp-eint", (void*) &sdp_eint[0]);
	if(retval) {
