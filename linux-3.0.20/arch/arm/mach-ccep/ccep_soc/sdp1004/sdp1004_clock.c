/*
 * linux/arch/arm/mach-sdp/sdp1004_clock.c
 *
...
unsigned long sdp1004_get_clock (char mode)
{
// 20080524 by tukho.kim 
// apply HRTIMER 
	if(!fclk.rate) sdp1004_init_clock();
...
static unsigned int __init sdp1004_calc_cpu_pms(unsigned int pms)
{
	unsigned int ret;
...
static unsigned int __init sdp1004_calc_ddr_pmsk(unsigned int pms, unsigned int k)
{
	unsigned int ret;
...
static int __init sdp1004_init_clock(void)
{
...
// apply HRTIMER 
	...
	fclk.rate = sdp1004_calc_cpu_pms(pms_core);
	dclk.rate = sdp1004_calc_ddr_pmsk(pms_ddr, k_ddr & 0xfff);

	busclk.rate = dclk.rate >> 1;
	hclk.rate = pclk.rate = dclk.rate >> 2;
...
int __init clk_init(void)
{
	...
	printk(KERN_INFO "sdp1004 clock control (c) 2010 Samsung Electronics\n");
	...
	for(i= 0; i< ARRAY_SIZE(init_clocks);i++) {
		clk = init_clocks[i];
		clk_register(clk);
		printk (KERN_INFO "SDP1004 %s Clock: %d.%03dMhz\n",
				clk->name,
...
EXPORT_SYMBOL(sdp1004_get_clock); 
#endif
