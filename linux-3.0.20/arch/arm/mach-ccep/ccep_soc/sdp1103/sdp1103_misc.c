/*
 * sdp1103_misc.c
 * sdp1103 miscellaneous (ext interrupt, pad mux, and pin attribute)
 *
...
//			printk(KERN_ERR"status:0x%x \n",status);
	...
	*reg_status = status;
//	printk(KERN_ERR"reg_status:0x%x \n",reg_status);
	return IRQ_HANDLED;
}
...
int sdp_subint_enable(int n_extirq)
{
	...
	R_SUB_MASK_MASK(n_extirq)&=~(0x3F);	//enable
...
int sdp_subint_disable(int n_extirq)
{
	...
	R_SUB_MASK_MASK(n_extirq)|=0x3F;//masking
	R_SUBINT_STAT|=(1<<n_extirq);
...
static int __init sdp_intcmerg_init(void)
{
	...
	printk(KERN_INFO"sdp  merger interrupt init\n");
...
static int __init sdp_subint_init(void)
{
	...
	retval = request_irq(31, sdp_subint_isr, IRQF_DISABLED, "sdp-subint", (void*) &sdp_subint[0]);//SUB 31 0~1
	if(retval) {
		printk(KERN_ERR"[%s] %d request_irq is failed\n", SUB_INTR_NAME, 31);
		goto __extint_exit;	
	} else {
