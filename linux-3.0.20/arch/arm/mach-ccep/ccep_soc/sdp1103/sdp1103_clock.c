/*
 * linux/arch/arm/mach-sdp/sdp1103_clock.c
 *
...
static int sdp1103_init_clock(void);
...
unsigned long sdp1103_get_clock (char mode)
{
// 20080524 by tukho.kim 
// apply HRTIMER 
	if(!fclk.rate) sdp1103_init_clock();
...
static unsigned int sdp1103_calc_cpu_pms(unsigned int pms)
{
	unsigned int ret;
...
static unsigned int sdp1103_calc_bus_pms(unsigned int pms)
{
	unsigned int ret;
...
static unsigned int sdp1103_calc_ddr_pmsk(unsigned int pms, unsigned int k)
{
	unsigned int ret;
...
// apply HRTIMER 
	...
	fclk.rate = sdp1103_calc_cpu_pms(pms_core);
	dclk.rate = sdp1103_calc_ddr_pmsk(pms_ddr, k_ddr & 0xfff);
	busclk.rate = sdp1103_calc_bus_pms(pms_bus);
...
int __init clk_init(void)
{
	...
	printk(KERN_INFO "sdp1103 clock control (c) 2010 Samsung Electronics\n");
	sdp1103_init_clock();
	//	return 0;
	...
	for(i= 0; i< ARRAY_SIZE(init_clocks);i++) {
		clk = init_clocks[i];
		clk_register(clk);
		printk (KERN_INFO "SDP1103 %s Clock: %d.%03dMhz\n",
				clk->name,
