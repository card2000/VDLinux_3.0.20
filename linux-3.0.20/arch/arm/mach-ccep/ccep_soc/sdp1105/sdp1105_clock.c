/*
 * linux/arch/arm/mach-sdp/sdp1105_clock.c
 *
...
static int sdp1105_init_clock(void);
...
unsigned long sdp1105_get_clock (char mode)
{
// 20080524 by tukho.kim 
// apply HRTIMER 
	if(!fclk.rate) sdp1105_init_clock();
...
static unsigned int __init sdp1105_calc_cpu_pms(unsigned int pms)
{
	unsigned int ret;
...
static unsigned int __init sdp1105_calc_ddr_pmsk(unsigned int pms, unsigned int k)
{
	unsigned int ret;
...
static unsigned int __init sdp1105_calc_dsp_pms(unsigned int pms)
{
	unsigned int ret;
	ret = (INPUT_FREQ >> GET_PLL_S(pms)) / GET_PLL_P(pms);
	ret *= GET_PLL_M(pms);
...
static int __init sdp1105_init_clock(void)
{
	unsigned int pms_core = R_PMU_CPU_PMS_CON;
	unsigned int pms_ddr = R_PMU_DDR_PMS_CON;
	unsigned int k_ddr = R_PMU_DDR_K_CON;
	unsigned int pms_dsp = R_PMU_DSP_PMS_CON;

// 20080524 by tukho.kim 
// apply HRTIMER 
	if(fclk.rate) return -1;

	fclk.rate = sdp1105_calc_cpu_pms(pms_core);
	dclk.rate = sdp1105_calc_ddr_pmsk(pms_ddr, k_ddr);
	dspclk.rate = sdp1105_calc_dsp_pms(pms_dsp);

	busclk.rate = dspclk.rate / 3;
	hclk.rate = pclk.rate = dspclk.rate >> 3; // dsp / 8
...
int __init clk_init(void)
{
	...
	printk(KERN_INFO "sdp1105 clock control (c) 2011 Samsung Electronics\n");
	sdp1105_init_clock();
	
	for(i= 0; i< ARRAY_SIZE(init_clocks);i++) {
		clk = init_clocks[i];
		clk_register(clk);
		printk (KERN_INFO "SDP1105 %s Clock: %d.%03dMhz\n",
				clk->name,
