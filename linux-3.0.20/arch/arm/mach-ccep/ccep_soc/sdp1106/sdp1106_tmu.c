/* linux/arch/arm/mach-ccep/ccep_soc/sdp1106/sdp1106_tmu.c
*
...
#define LIMIT_1ST_THROTTLE	500000
#define LIMIT_2ND_THROTTLE	250000

#if 0
#define STOP_1ST_THROTTLE   65
#define START_1ST_THROTTLE  70
#define STOP_2ND_THROTTLE   75
#define START_2ND_THROTTLE  80
#define START_TRIPPING	    85
#define START_EMERGENCY     90
#else
#define STOP_1ST_THROTTLE   150
#define START_1ST_THROTTLE  160
#define STOP_2ND_THROTTLE   170
#define START_2ND_THROTTLE  180
#define START_TRIPPING	    190
#define START_EMERGENCY     200
#endif
#define SAMPLING_RATE		(4 * 1000 * 1000) /* u sec */
...
#define SA56004X_CAHNNEL	(0)
#define SA56004X_SLAVE_ADDR	(0x90)
#define SA56004X_LTHB_REG	(0x0)
#define SA56004X_RTHB_REG	(0x1)
static unsigned int get_curr_temp(void)
{
	unsigned char buff = 0;//, buff2 = 0;
	
	struct sdp_i2c_packet_t	packet; 
	unsigned char subaddr;

	/* SA56004X external sensor local */
	subaddr = SA56004X_LTHB_REG;
	packet.slaveAddr	= SA56004X_SLAVE_ADDR; // sa56004x address
...
//	sdp_i2c_request(SA56004X_CAHNNEL, I2C_CMD_READ, &packet);

	/* SA56004X external sensor remote */
//	subaddr = SA56004X_RTHB_REG;
//	packet.pSubAddr 	= &subaddr;
//	packet.pDataBuffer	= &buff2;

//	sdp_i2c_request(SA56004X_CAHNNEL, I2C_CMD_READ, &packet);

#if 0
	printk("\033[33mLocal Temp = %d\033[0m\n", (int)buff);
//	printk("\033[33mRemote Temperature = %d\033[0m\n", (int)buff2);
#endif	
...
#if 0//defined(CONFIG_TC_VOLTAGE)
/**
 * sdp_tc_volt - locks or frees vdd_arm, vdd_mif/int and vdd_g3d for
 * temperature compensation.
 *
 * This function limits or free voltage of cpufreq, busfreq, and mali driver
 * according to 2nd arguments.
 */
static int sdp_tc_volt(struct sdp_tmu_info *info, int enable)
{
	static int usage;
	int ret = 0;
	...
	if (enable) {
		ret = sdp_cpufreq_lock(DVFS_LOCK_ID_TMU, info->cpulevel_tc);
		if (ret)
			goto err_lock;
	} else {
		sdp_cpufreq_lock_free(DVFS_LOCK_ID_TMU);
	}
	usage = enable;
	pr_info("TMU: %s is ok!\n", enable ? "lock" : "unlock");
	return ret;
...
//	static int auto_refresh_changed;
	...
	case TMU_STATUS_NORMAL:
		...
		if (cur_temp >= START_1ST_THROTTLE) {
			info->tmu_state = TMU_STATUS_THROTTLED;
			printk("change state: normal->throttle.\n");
#if 0//defined(CONFIG_TC_VOLTAGE)
		...
		} else if (cur_temp <= STOP_1ST_THROTTLE) {
...
//			notify_change_of_tmu_state(info);
			//printk("normal: free cpufreq_limit & interrupt enable.\n");
...
//			enable_irq(info->irq);
	...
	case TMU_STATUS_THROTTLED:
		...
		if (cur_temp >= START_2ND_THROTTLE) {
			info->tmu_state = TMU_STATUS_WARNING;
			printk("change state: 1st throttle->2nd throttle.\n");
#if 0//defined(CONFIG_TC_VOLTAGE)
		/* check whether temp compesation need or not */
		} else if (cur_temp <= data->ts.start_tc) {
			if (sdp_tc_volt(info, 1) < 0)
		...
		/* 2. cpufreq limitation and uevent */
		} else if ((cur_temp >= START_1ST_THROTTLE) &&
			!(check_handle & THROTTLE_FLAG)) {
			...
			pr_debug("check_handle = %d\n", check_handle);
//			notify_change_of_tmu_state(info);
			printk("throttling: set cpufreq upper limit.\n");
		/* 3. change state: normal */
		} else if ((cur_temp <= STOP_1ST_THROTTLE)
			&& (trend < 0)) {
			info->tmu_state = TMU_STATUS_NORMAL;
			printk("change state: 1st throttle->normal.\n");
		}
		break;

	case TMU_STATUS_WARNING:
		/* 1. change state: tripping */
		if (cur_temp >= START_TRIPPING) {
			info->tmu_state = TMU_STATUS_TRIPPED;
			printk("change state: 2nd throttle->trip\n");
#if 0//defined(CONFIG_TC_VOLTAGE)
		/* check whether temp compesation need or not */
		} else if (cur_temp <= data->ts.start_tc) {
			if (sdp_tc_volt(info, 1) < 0)
...
#endif
		/* 2. cpufreq limitation and uevent */
		} else if ((cur_temp >= START_2ND_THROTTLE) &&
			!(check_handle & WARNING_FLAG)) {
			if (check_handle & THROTTLE_FLAG) {
				sdp_cpufreq_upper_limit_free(DVFS_LOCK_ID_TMU);
				check_handle &= ~(THROTTLE_FLAG);
			...
			pr_debug("check_handle = %d\n", check_handle);
//			notify_change_of_tmu_state(info);
			printk("2nd throttle: cpufreq is limited.\n");
		/* 3. change state: 1st-throttling */
		} else if ((cur_temp <= STOP_2ND_THROTTLE)
			&& (trend < 0)) {
			info->tmu_state = TMU_STATUS_THROTTLED;
			printk("change state: 2nd throttle->1st throttle, "
...
//			notify_change_of_tmu_state(info);
			printk("tripping: on waiting shutdown.\n");
...
//		notify_change_of_tmu_state(info);
		printk("%s: inform to init state to platform.\n", __func__);
		break;
...
static int sdp1106_tmu_init(struct sdp_tmu_info *info)
{
//	struct sdp_platform_tmu *data = info->dev->platform_data;
//	unsigned int tmp;
...
static irqreturn_t exynos4x12_tmu_irq_handler(int irq, void *id)
{
	...
	*/
	...
	info->last_temperature =  get_curr_temp();
...
//	struct resource *res;
	...
	sdp_cpufreq_get_level(LIMIT_1ST_THROTTLE,
			&info->cpufreq_level_1st_throttle);

	sdp_cpufreq_get_level(LIMIT_2ND_THROTTLE,
			&info->cpufreq_level_2nd_throttle);
...
#if 0//defined(CONFIG_TC_VOLTAGE) /* Temperature compensated voltage */
...
//			IRQF_DISABLED,  "sdp-tmu interrupt", info);
//	if (ret) {
...
//	}
	...
	*/
	if (get_curr_temp() <= pdata->ts.start_tc) {
		if (sdp_tc_volt(info, 1) < 0)
			pr_err("TMU: lock error!\n");
	}
...
static int sdp_tmu_resume(struct platform_device *pdev)
{
	...
	*/
	mdelay(1);
	if (get_curr_temp() <= data->ts.start_tc) {
		if (sdp_tc_volt(info, 1) < 0)
			pr_err("TMU: lock error!\n");
	}
#endif
...
late_initcall(sdp_tmu_driver_init);
