/*
 * linux/arch/arm/mach-sdp/sdp1106_clock.c
 *
...
static int sdp1106_init_clock(void);
...
unsigned long sdp1106_get_clock (char mode)
{
// 20080524 by tukho.kim 
// apply HRTIMER 
	if(!fclk.rate) sdp1106_init_clock();
...
static unsigned int __init sdp1106_calc_cpu_pms(unsigned int pms)
{
	unsigned int ret;
...
static unsigned int __init sdp1106_calc_bus_pms(unsigned int pms)
{
	unsigned int ret;
	ret = (INPUT_FREQ >> (GET_PLL_S(pms))) / GET_PLL_P(pms);
	ret *= GET_PLL_M(pms); 
...
static unsigned int __init sdp1106_calc_ddr_pmsk(unsigned int pms, unsigned int k)
{
	unsigned int ret;
...
static int __init sdp1106_init_clock(void)
{
...
// apply HRTIMER 
	...
	fclk.rate = sdp1106_calc_cpu_pms(pms_core);
	dclk.rate = sdp1106_calc_ddr_pmsk(pms_ddr, k_ddr);
	busclk.rate = sdp1106_calc_bus_pms(pms_bus);
	pclk.rate = busclk.rate / 2;
	hclk.rate = busclk.rate / 2;
...
int __init clk_init(void)
{
	...
	printk(KERN_INFO "sdp1106 clock control (c) 2011 Samsung Electronics\n");
	sdp1106_init_clock();
	//	return 0;
	...
	for(i= 0; i< ARRAY_SIZE(init_clocks);i++) {
		clk = init_clocks[i];
		clk_register(clk);
		printk (KERN_INFO "SDP1106 %s Clock: %d.%03dMhz\n",
				clk->name,
