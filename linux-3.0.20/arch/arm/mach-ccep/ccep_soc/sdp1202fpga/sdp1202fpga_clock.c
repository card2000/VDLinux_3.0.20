/*
 * linux/arch/arm/mach-sdp/sdp1202fpga_clock.c
 *
 * Copyright (C) 2011 Samsung Electronics.co
 * Author : sh.chon@samsung.com 	14/11/2011
 *
 */
...
static int sdp1202fpga_init_clock(void);
...
unsigned long sdp1202fpga_get_clock (char mode)
{
// 20080524 by tukho.kim 
// apply HRTIMER 
	if(!fclk.rate) sdp1202fpga_init_clock();
...
static int __init sdp1202fpga_init_clock(void)
{
	if(fclk.rate) return -1;

	fclk.rate = 0;
	dclk.rate = 0;
	busclk.rate = CONFIG_FPGA_BUSCLKSRC / 8;
...
int __init clk_init(void)
{
	...
	printk(KERN_INFO "sdp1202fpga clock control (c) 2011 Samsung Electronics\n");
	sdp1202fpga_init_clock();

	for(i= 0; i< ARRAY_SIZE(init_clocks);i++) {
		clk = init_clocks[i];
		clk_register(clk);
		printk (KERN_INFO "sdp1202fpga %s Clock: %d.%03dMhz\n",
				clk->name,
