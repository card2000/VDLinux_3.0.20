...
-------------------------------------------------------------------------------*/
void edison_irq_ack(unsigned int irq);
void edison_irq_mask(unsigned int irq);
void edison_irq_unmask(unsigned int irq);
void __init edison_init_irq(void);

/* Clear FIQ (Clear is not supported for IRQ) */
void edison_irq_ack(unsigned int irq) {
    unsigned short tmp;
//printk(KERN_WARNING "edison_irq_ack(irq=%d)\n",irq);
        if(irq <16)
	{
...
void edison_irq_mask(unsigned int irq) {
    unsigned short tmp;
//printk(KERN_WARNING "edison_irq_mask(irq=%d)\n",irq);
...
void edison_irq_unmask(unsigned int irq) {
   unsigned short tmp;
    ...
    //printk(KERN_WARNING "edison_irq_unmask(irq=%d)\n",irq);
...
void __init edison_init_irq(void)
{
    ...
    gic_init(0,29,_gic_dist_base_addr,_gic_cpu_base_addr);
       ...
       temp=(unsigned long)PERI_R(GIC_DIST_SET_EANBLE);
       temp= temp | (unsigned long)(0x1 << INT_PPI_IRQ );
