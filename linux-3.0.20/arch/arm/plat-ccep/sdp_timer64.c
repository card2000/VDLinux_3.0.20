/*
 * linux/arch/arm/plat-ccep/sdp_timer64.c
 *
...
static unsigned long sdp_gettimeoffset (void)
{
	...
	unsigned long nsec;
	unsigned int n_past;
	unsigned int cur_count = R_SYSTMCNT64L;
...
static struct irqaction sdp_timer_irq = {
	...
	.flags	= IRQF_SHARED | IRQF_DISABLED | IRQF_TRIGGER_RISING,
#else
...
static void __init sdp_get_time_tick(unsigned long* max_value)
{
	...
	if(init_val_sys_tmcnt == 0){
		...
		timer_source = timer_source >> 1; //prescaler value is fixed to '1' -> divide 2

		init_val_sys_tmcnt = timer_source / nsec;
	...
	printk("Samsung SDP Timer Clock: %d.%03d usec per tick\n"
			"\tLoad value=%d, Tick HZ = %d\n", 
			(int)usec_per_clock.usec, (int)usec_per_clock.nsec, init_val_sys_tmcnt, SYS_TICK);	
