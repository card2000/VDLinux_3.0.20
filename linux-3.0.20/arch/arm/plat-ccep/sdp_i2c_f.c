...
// Oct,08,2009
	// 1.211
//	pPacket->reserve8 &= ~(MODE_SCL_MON_OFF);  	// 110920 always scl mon on
	if(pPacket->reserve8 & MODE_SCL_MON_OFF){ 
		R_I2C_CONE &= ~(0x100);	// 110920 mon funcion on
	} else {
		R_I2C_CONE |= 0x100;	// 1.211, default mon funcion off 
	}
// Oct,08,2009 end
