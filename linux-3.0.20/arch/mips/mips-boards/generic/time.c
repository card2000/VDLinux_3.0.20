...
static unsigned int mips_cpu_perf_irq;
...
static void __init plat_perf_setup(void)
{
	...
	if (cp0_perfcount_irq >= 0) {
		...
		mips_cpu_perf_irq = MIPS_CPU_IRQ_BASE + (unsigned int)cp0_perfcount_irq;
#ifdef CONFIG_SMP
...
unsigned int __cpuinit get_c0_compare_int(void)
{
...
#endif
	{
		...
		mips_cpu_timer_irq = MIPS_CPU_IRQ_BASE + (unsigned int)cp0_compare_irq;
	}
...
void __init plat_time_init(void)
{
	...
	printk("CPU frequency %d.%02d MHz\n", est_freq/1000000,
        ...
        /* Mstar Chip does not use this function */
        //mips_scroll_message();
