...
static int Mstar_Chip_EnableInterrupt (InterruptNum eIntNum)
{
    ...
    {
            REG(REG_IRQ_MASK_L) &= (unsigned int)(~IRQL_MASK);
            REG(REG_IRQ_MASK_H) &= (unsigned int)(~IRQH_MASK);
            REG(REG_FIQ_MASK_L) &= (unsigned int)(~FIQL_MASK);
            REG(REG_FIQ_MASK_H) &= (unsigned int)(~FIQH_MASK);

            REG(REG_IRQ_EXP_MASK_L) &= (unsigned int)(~IRQEXPL_MASK);
            REG(REG_IRQ_EXP_MASK_H) &= (unsigned int)(~IRQEXPH_MASK);
            REG(REG_FIQ_EXP_MASK_L) &= (unsigned int)(~FIQEXPL_MASK);
            REG(REG_FIQ_EXP_MASK_H) &= (unsigned int)(~FIQEXPH_MASK);
    }
    else if ( (eIntNum >= E_IRQL_START) && (eIntNum <= E_IRQL_END) )
    {
            REG(REG_IRQ_MASK_L) &= (unsigned int)(~(0x1 << (eIntNum-E_IRQL_START)) );
    }
    else if ( (eIntNum >= E_IRQH_START) && (eIntNum <= E_IRQH_END) )
    {
            REG(REG_IRQ_MASK_H) &= (unsigned int)(~(0x1 << (eIntNum-E_IRQH_START)) );
    }
    else if ( (eIntNum >= E_FIQL_START) && (eIntNum <= E_FIQL_END) )
    {
            REG(REG_FIQ_MASK_L) &= (unsigned int)(~(0x1 << (eIntNum-E_FIQL_START)) );
    }
    else if ( (eIntNum >= E_FIQH_START) && (eIntNum <= E_FIQH_END) )
    {
            REG(REG_FIQ_MASK_H) &= (unsigned int)(~(0x1 << (eIntNum-E_FIQH_START)) );
    }
    else if ( (eIntNum >= E_IRQEXPL_START) && (eIntNum <= E_IRQEXPL_END) )
    {
            REG(REG_IRQ_EXP_MASK_L) &= (unsigned int)(~(0x1 << (eIntNum-E_IRQEXPL_START)) );
    }
    else if ( (eIntNum >= E_IRQEXPH_START) && (eIntNum <= E_IRQEXPH_END) )
    {
            REG(REG_IRQ_EXP_MASK_H) &= (unsigned int)(~(0x1 << (eIntNum-E_IRQEXPH_START)) );
    }
    else if ( (eIntNum >= E_FIQEXPL_START) && (eIntNum <= E_FIQEXPL_END) )
    {
            REG(REG_FIQ_EXP_MASK_L) &= (unsigned int)(~(0x1 << (eIntNum-E_FIQEXPL_START)) );
    }
    else if ( (eIntNum >= E_FIQEXPH_START) && (eIntNum <= E_FIQEXPH_END) )
    {
            REG(REG_FIQ_EXP_MASK_H) &= (unsigned int)(~(0x1 << (eIntNum-E_FIQEXPH_START)) );
    }
    return bRet;
}
...
static int Mstar_Chip_DisableInterrupt (InterruptNum eIntNum)
{
    ...
    {
        REG(REG_FIQ_MASK_L) |= (unsigned int)(0x1 << (eIntNum-E_FIQL_START) );
        REG(REG_FIQ_CLEAR_L) = (unsigned int)(0x1 << (eIntNum-E_FIQL_START) );
    }
    else if ( (eIntNum >= E_FIQH_START) && (eIntNum <= E_FIQH_END) )
    {
        REG(REG_FIQ_MASK_H) |= (unsigned int)(0x1 << (eIntNum-E_FIQH_START) );
        REG(REG_FIQ_CLEAR_H) = (unsigned int)(0x1 << (eIntNum-E_FIQH_START) );
    }
    else if ( (eIntNum >= E_IRQEXPL_START) && (eIntNum <= E_IRQEXPL_END) )
    {
        REG(REG_IRQ_EXP_MASK_L) |= (unsigned int)(0x1 << (eIntNum-E_IRQEXPL_START) );
    ...
    {
        REG(REG_FIQ_EXP_MASK_L) |= (unsigned int)(0x1 << (eIntNum-E_FIQEXPL_START) );
        REG(REG_FIQ_EXP_CLEAR_L) = (unsigned int)(0x1 << (eIntNum-E_FIQEXPL_START) );
    }
    else if ( (eIntNum >= E_FIQEXPH_START) && (eIntNum <= E_FIQEXPH_END) )
    {
        REG(REG_FIQ_EXP_MASK_H) |= (unsigned int)(0x1 << (eIntNum-E_FIQEXPH_START) );
        REG(REG_FIQ_EXP_CLEAR_H) = (unsigned int)(0x1 << (eIntNum-E_FIQEXPH_START) );
    }
    return 0;
}
...
void Mstar_Chip_hw0_irqdispatch(void)
{
    ...
    while (32> (u32Bit = MAsm_CPU_GetTrailOne(u16Reg)))
    {
        ...
        u16Reg &= (unsigned int)(~(0x1<< u32Bit));
    }
...
static inline unsigned int irq_ffs(unsigned int pending)
{
	...
	return (unsigned int)(-clz(pending) + 31 - CAUSEB_IP);
#else
	unsigned int a0 = 7;
	unsigned int t0;
