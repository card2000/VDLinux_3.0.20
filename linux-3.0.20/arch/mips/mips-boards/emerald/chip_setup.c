...
extern int	internal_micom_model;
...
#if defined(CONFIG_MSTAR_EMERALD_BD_FPGA)
...
static void __init serial_init(void)
{
	...
	// Morris.Cheng 2010/07/29 for X6 uart2 (third uart mode)
	memset(&u, 0, sizeof(u));

	u.type = PORT_16550;
	u.iobase = 0xBF220D00;
	u.irq = E_IRQEXPH_UART2MCU;
	u.uartclk = 123000000;
	u.iotype = 0;
	u.regshift = 0;
	u.fifosize = 16 ; // use the 8 byte depth FIFO well
...
void uart_clk_scan(void)
{
    ...
    {
        unsigned int divisor= (clk[i]/(16 * MSTAR_CHIP_UART_BAUD_RATE));
        //UART1 INIT:
...
//   Select UART2 source
    ...
    *(volatile volatile unsigned int*)(0xbf203c08) &= 0xFBFF;
    ...
    UART_REG(MSTAR_CHIP_UART_LCR) &= (unsigned int)(~(0x80)) ;
...
{
    volatile __attribute__((unused))unsigned int temp=0;
...
////        u32L2CacheLineMask = ~(CacheLineSize-1);
////    }
////}
////
////if(  *(volatile unsigned int *)(0xbf000000+0x20310c) & _BIT(0) )  // H43 enable write through
////{
////    u32L2WriteThrough=1;
...
////}
////
////if(  *(volatile unsigned int*)(0xbf000000+0x203100) & _BIT(2) ) // H40
////{
////    u32L2AllInCache=1;
////    printk( "(all)\n"  );
////}
