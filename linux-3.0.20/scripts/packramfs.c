...
int main( int argc, char *argv[] )
{
	...
	sprintf( cmd_str, "arch/mips/boot/mkimage -n 'linux-3.0.20' -A mips -O linux -T kernel -C none -a 0x%08X -e 0x%08X -d vmlinux.bin vmlinux.img", (unsigned int)ker_dst_base, (unsigned int)ker_dst_entry );
	system( cmd_str );
	...
	sprintf( cmd_str, "arch/mips/boot/mkimage -n 'initramfs' -A mips -O linux -T ramdisk -C none -a 0x%08X -d initramfs.bin initramfs.img", stHead.initramfs_start - sizeof( ST_INITRAMFS_HEAD ) );
	system( cmd_str );
	printf( "\r\n");
