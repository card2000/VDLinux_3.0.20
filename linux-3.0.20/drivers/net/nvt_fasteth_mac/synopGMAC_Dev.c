...
void synopGMAC_fast_ethernet_speed(synopGMACdevice *gmacdev, u32 speed);
...
// jay hsu : set ahb clock by auto detect function @ 20100329
addr = ((PhyBase << GmiiDevShift) & GmiiDevMask) | ((RegOffset << GmiiRegShift) & GmiiRegMask) | (u32)s32MDC_CLOCK;
...
// jay hsu : set ahb clock by auto detect function @ 20100329
addr = ((PhyBase << GmiiDevShift) & GmiiDevMask) | ((RegOffset << GmiiRegShift) & GmiiRegMask) | GmiiWrite | (u32)s32MDC_CLOCK;
...
s32 synopGMAC_get_phy_id(synopGMACdevice * gmacdev)
{
	u16 data;
	s32 phy_id=0;
	
	synopGMAC_read_phy_reg((u32 *)gmacdev->MacBase,gmacdev->PhyBase,PHY_ID_HI_REG, &data);
	phy_id = data << 16;
	synopGMAC_read_phy_reg((u32 *)gmacdev->MacBase,gmacdev->PhyBase,PHY_ID_LOW_REG, &data);
	phy_id += data;
...
s32 synopGMAC_reset_phy(synopGMACdevice * gmacdev)
{
	u16 data;
	
	s32 status = -ESYNOPGMACNOERR;
	status = synopGMAC_write_phy_reg((u32 *)gmacdev->MacBase,gmacdev->PhyBase,PHY_CONTROL_REG, 	Mii_reset );
	if (status)
		return status;
...
s32 synopGMAC_check_phy_init_for_10_100M(synopGMACdevice * gmacdev)
{
	//u8 auto_negotiation = 0;
	u16 data;
	s32 status = -ESYNOPGMACNOERR;		
	s32 loop_count = DEFAULT_LOOP_VARIABLE;
	
	// Jay Hsu @ 20100707 : new method to check 10/100M phy status
	...
	// jay hsu : check auto-negotiation result
	if ( loop_count < 0 )
	{
		printk("[NTKETHMAC] : Ethernet Phy Auto-Negotiation Failed\n");
		return -ESYNOPGMACPHYERR;
	}
	else		// jay hsu @ 20100707 : check negotiation result or setting by phy's strapping
	{				
			
#ifdef CONFIG_NVT_MICREL_PHY_SUPPORT	// the method of Micrel phy, why can't get the correct result on reg 0 bit13 and bit8.

		status = synopGMAC_read_phy_reg((u32 *)gmacdev->MacBase,gmacdev->PhyBase,PHY_CONTRL_2_REG, &data);
		if(status)	
			return status;
		...
		printk("[NTKETHMAC] : Set RTL phy into rising edge output\n");		
#endif	
...
s32 synopGMAC_set_mac_addr(synopGMACdevice *gmacdev, u32 MacHigh, u32 MacLow, u8 *MacAddr)
{
	...
	data = (u32)(MacAddr[5] << 8) | MacAddr[4];
	synopGMACWriteReg((u32 *)gmacdev->MacBase,MacHigh,data);
	data = (u32)(MacAddr[3] << 24) | (u32)(MacAddr[2] << 16) | (u32)(MacAddr[1] << 8) | MacAddr[0] ;
...
s32 synopGMAC_get_tx_qptr(synopGMACdevice * gmacdev, u32 * Status, u32 * Buffer1, u32 * Length1, u32 * Data1, u32 * Buffer2, u32 * Length2, u32 * Data2 )
{
	...
	dma_cache_sync(NULL, txdesc,sizeof(DmaDesc), DMA_FROM_DEVICE);	// jay debug : use non-cache
	...
	// jay hsu : do cache sync, cache sync to dram
	dma_cache_sync(NULL, txdesc,sizeof(DmaDesc), DMA_TO_DEVICE);	// jay hsu @ 20100825 : add cache sync to dram
...
//		synopGMAC_tx_checksum_offload_tcp_pseudo(gmacdev, txdesc);
	...
	dma_cache_sync(NULL, phys_to_virt(Buffer1),Length1, DMA_TO_DEVICE);
	dma_cache_sync(NULL, txdesc,sizeof(DmaDesc), DMA_TO_DEVICE);	// jay debug : use non-cache
...
s32 synopGMAC_set_rx_qptr(synopGMACdevice * gmacdev, u32 Buffer1, u32 Length1, u32 Data1, u32 Buffer2, u32 Length2, u32 Data2)
{
	...
	//dma_cache_sync(NULL, rxdesc,sizeof(DmaDesc), DMA_FROM_DEVICE);	// jay hsu @ 20100819 : don't need to do cache sync
	...
	// jay hsu : do cache sync, cpu cache sync to dram
	//dma_cache_sync(NULL, (u32 *)Buffer1,Length1, DMA_TO_DEVICE); // jay hsu @ 20100820, can ignore to write back
	dma_cache_sync(NULL, rxdesc,sizeof(DmaDesc), DMA_TO_DEVICE);	// jay debug : use non-cache
...
s32 synopGMAC_get_rx_qptr(synopGMACdevice * gmacdev, u32 * Status, u32 * Buffer1, u32 * Length1, u32 * Data1, u32 * Buffer2, u32 * Length2, u32 * Data2)
{
	...
	// jay hsu : do cache sync, dram sync to cpu cache
	dma_cache_sync(NULL, rxdesc,sizeof(DmaDesc), DMA_FROM_DEVICE);	// jay debug : use non-cache
	...
	TR("jay hsu : debug Buffer1 0x%x, length %d\n", rxdesc->buffer1, ((rxdesc->length & DescSize1Mask) >> DescSize1Shift));
	// jay hsu : do cache sync, dram sync to cpu cache
	dma_cache_sync(NULL, phys_to_virt(rxdesc->buffer1),((rxdesc->length & DescSize1Mask) >> DescSize1Shift), DMA_FROM_DEVICE);	
...
{
	#ifdef ENH_DESC
	desc->status = (desc->length & (u32)(~DescTxCisMask));//ENH_DESC
	#else
...
{
	#ifdef ENH_DESC
	desc->status = ((desc->status & (u32)(~DescTxCisMask)) | DescTxCisIpv4HdrCs);//ENH_DESC
	#else
	desc->length = ((desc->length & (u32)(~DescTxCisMask)) | DescTxCisIpv4HdrCs);
	#endif
...
{
	#ifdef ENH_DESC
	desc->status = ((desc->status & (u32)(~DescTxCisMask)) | DescTxCisTcpOnlyCs);//ENH_DESC
	#else
	desc->length = ((desc->length & (u32)(~DescTxCisMask)) | DescTxCisTcpOnlyCs);
	#endif
...
{
	#ifdef ENH_DESC
	desc->status = ((desc->length & (u32)(~DescTxCisMask)) | DescTxCisTcpPseudoCs);
	#else
	desc->length = ((desc->length & (u32)(~DescTxCisMask)) | DescTxCisTcpPseudoCs);
	#endif
...
#if 0
// jay hsu : force Micrel KSZ8041NL phy in 100Mbps half-duplex mode for vicki streaming
s32 synopGMAC_set_phy_in_100M(synopGMACdevice * gmacdev)
{
	u16 data;
	
	printk("jay debug : set 100M half-duplex\n");
	synopGMAC_read_phy_reg((u32 *)gmacdev->MacBase,gmacdev->PhyBase,PHY_CONTROL_REG, &data);
	msleep(100);
	synopGMAC_write_phy_reg((u32 *)gmacdev->MacBase,gmacdev->PhyBase,PHY_CONTROL_REG, (data & (~(1<<12|1<<8)) ) );

	return -ESYNOPGMACNOERR;	
}

// jay hsu : force RTL phy in 100Mbps full-duplex mode
s32 synopGMAC_set_rtl_phy_in_100M(synopGMACdevice * gmacdev)
{
	u16 data;
