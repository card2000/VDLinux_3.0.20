...
extern u8 isFirstInquiryTimeout;	//for HD+bad cable, Colin, 090216
	...
	void *buf, unsigned int length, unsigned int *act_len)
{
	...
	US_DEBUGP("%s: xfer %u bytes\n", __func__, length);
        ...
        //patch 20070424 for SDCZ2 Cruzer Mini Flash Drive (older, thick)
	...
	if (result < 0) {
		US_DEBUGP("Soft reset failed: %d\n", result);
                ...
                if(result == -ECONNRESET)
                        set_bit(US_FLIDX_CONNRESET, &us->dflags);
#endif
		return result;
	}
