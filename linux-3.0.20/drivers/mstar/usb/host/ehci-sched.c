...
static int enable_periodic (struct ehci_hcd *ehci)
{
	...
	status = handshake_on_error_set_halt(ehci, &ehci->regs->status,
					     STS_PSS, 0, 9 * 125);
	if (status) {
#if 0	//20121123, for unplug webcam when it is working
...
)
{
	...
	if (dev->speed == USB_SPEED_HIGH) {
		...
		if ((ehci_readl(ehci, &ehci->regs->bmcs) & 0x0600) != 0x0400 &&
			(dev->speed != USB_SPEED_HIGH)) //Colin, patch for not real split-transaction mode
		{
			unsigned multi = 1;
...
/* tony.yu map between PHY addr & BUS addr */
				...
				if ((ehci_readl(ehci, &ehci->regs->bmcs) & 0x0600) != 0x0400 &&
					(stream->udev->speed != USB_SPEED_HIGH)) //Colin, patch for not real split-transaction mode
				{
					struct ehci_itd		*itd;
...
)
{
	...
	if ((ehci_readl(ehci, &ehci->regs->bmcs) & 0x0600) != 0x0400 &&
		(urb->dev->speed != USB_SPEED_HIGH)) //Colin, patch for not real split-transaction mode
	{
...
)
{
	...
	if ((ehci_readl(ehci, &ehci->regs->bmcs) & 0x0600) != 0x0400 &&
		(urb->dev->speed != USB_SPEED_HIGH)) //Colin, patch for not real split-transaction mode
		next_uframe = (unsigned)stream->next_uframe;
	else
	...
	if ((ehci_readl(ehci, &ehci->regs->bmcs) & 0x0600) != 0x0400 &&
		(urb->dev->speed != USB_SPEED_HIGH)) //Colin, patch for not real split-transaction mode
		stream->next_uframe = (int)(next_uframe % mod);
	else
