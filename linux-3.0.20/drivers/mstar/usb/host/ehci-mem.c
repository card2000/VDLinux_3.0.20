...
//MStar, to force clean up the DMA Pool
...
static void ehci_force_cleanup_mem_pool(struct dma_pool *pool)
{
	struct list_head *entry, *tmp;

	list_for_each_safe (entry, tmp, &pool->page_list)
	{
...
static void ehci_mem_cleanup (struct ehci_hcd *ehci)
{
	...
	if (ehci->qtd_pool)
		ehci_force_cleanup_mem_pool (ehci->qtd_pool);
	ehci->qtd_pool = NULL;

	if (ehci->qh_pool) {
		ehci_force_cleanup_mem_pool (ehci->qh_pool);
		ehci->qh_pool = NULL;
	}
