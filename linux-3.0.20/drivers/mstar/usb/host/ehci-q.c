...
#if _USB_SPLIT_ZERO_PKG_BLOCKING_PATCH
...
#elif _USB_SPLIT_MDATA_BLOCKING_PATCH
			else if((token & QTD_STS_STS)					/* SplitXstate = 1 in qTD */
				&& (hc32_to_cpu(ehci, qh->hw->hw_info2) & QH_HUBADDR)	/* hub addr */
				&& (hc32_to_cpu(ehci, qh->hw->hw_info2) & QH_CMASK)	/* split complite mask */
				&& usb_pipeint(urb->pipe) && usb_pipein(urb->pipe)	/* Interrupt IN transaction */
...
qh_urb_transaction (
	...
	for (;;) {
		...
		if (is_input && usb_pipebulk (urb->pipe) && (!(urb->transfer_flags & URB_NO_INTERRUPT)) && (!(urb->transfer_flags & URB_SHORT_NOT_OK)))
		{
...
/*-------------------------------------------------------------------------*/
extern int UHC_getPortSC(int busnum);
