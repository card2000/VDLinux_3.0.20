...
extern unsigned char hcd_readb(struct usb_hcd *, unsigned int);
...
rescan:
	...
	while (!list_empty (&ep->urb_list)) {
		...
		if (urb) {
			usb_hcd_unlink_urb_from_ep(hcd,urb);	//Add by Austin for ISO hotlpug
			if(atomic_read(&urb->use_count) > 0)
				atomic_dec(&urb->use_count);
			usb_kill_urb (urb);
			usb_put_urb (urb);
		...
		unsigned int irqnum, unsigned long irqflags)
{
	...
	if (hcd->ehc_base!=0)
	{
		if (hcd_readb(hcd, 0x30) & 0x01) //120210, for port reset when connected at startup
