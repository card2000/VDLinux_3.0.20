...
//--------------------------------------------------------------------------------------------------
// lan loop back test variable
#define MAX_NAMELEN 10
#define BITS_PER_BYTE 8
static struct proc_dir_entry *proc_entry;
static struct proc_dir_entry *eth_dir;
struct sk_buff *test_packet;
...
0x00, 0x25, 0x64, 0x68, 0xdf, 0xde, 0x00, 0x1c, 0x73, 0x1c, 0x2d, 0x62, 0x08, 0x00, 0x45, 0x00,
0x00, 0x28, 0x0a, 0x35, 0x40, 0x00, 0x7f, 0x06, 0x39, 0x16, 0xac, 0x10, 0x21, 0x2f, 0xac, 0x10,
0x21, 0x2f, 0x01, 0xbd, 0x04, 0x18, 0x03, 0x81, 0xf3, 0xb8, 0xbb, 0x2b, 0xee, 0xe3, 0x50, 0x10,
0xfc, 0x37, 0x53, 0xf9, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00
};
//--------------------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------------------
// lan loop back test function
static int MDev_EMAC_loopback_test (void);
//-------------------------------------------------------------------------------------------------

//-------------------------------------------------------------------------------------------------
//  Local Function
//-------------------------------------------------------------------------------------------------
int loopback_pow(int n, int p);
int loopback_write( struct file *filp, const char *buff, unsigned long len, void *data );
int loopback_read( char *page, char **start, off_t off, int count, int *eof, void *data );
int init_lan_proc( void );
...
//--------------------------------------------------------------------------------------------------
/* Create lan loop back test proc entry */
int loopback_pow(int n, int p)
{
    int i, r = 1;
    for(i = 0; i < p; i++)
...
int loopback_write( struct file *filp, const char *buff, unsigned long len, void *data )
{
    char tmpbuf[64];
    int Tbuf[64];
    u32 cnt, shift = BITS_PER_BYTE*sizeof(u32);
    ...
    if (buff && !copy_from_user(tmpbuf, buff, len))
    {
        if(tmpbuf[0] != '0')
        {
            ...
            {
                Tbuf[cnt] = (int)tmpbuf[cnt] - '0';
                test_num += (Tbuf[cnt] * loopback_pow(10, (int)(len - cnt - 1)));
            }
            // start test
            MDev_EMAC_loopback_test();
        }
        else
        {
            // set packet address
		...
		vAddr = NULL;
                ...
                {
                    if(((int)tmpbuf[cnt] >= '0') && ((int)tmpbuf[cnt] <= '9'))
                        pAddr |= (((int)tmpbuf[cnt] - '0') << (shift -= 4));
                    else if(((int)tmpbuf[cnt] >= 'a') && ((int)tmpbuf[cnt] <= 'f'))
                        pAddr |= (((((int)tmpbuf[cnt] - '0') & 0xF) + 0x9) << (shift -= 4));
                    else if(((int)tmpbuf[cnt] >= 'A') && ((int)tmpbuf[cnt] <= 'F'))
                        pAddr |= (((((int)tmpbuf[cnt] - '0') & 0xF) + 0x9) << (shift -= 4));
                    else
                        printk(">>>>>>>>>>>>>Invild address<<<<<<<<<<<<<\n");
                }
                vAddr = (int *)ioremap((unsigned long)pAddr, sizeof(TCP_packet)); //map buncing buffer from PA to VA
            }
            // end test
            else
            {
	        kfree_skb(test_packet);
		...
		test_num = 0;	
                PassFlag = (loop_back_test == 0 ? '0' : '1');

                copy_to_user((void *)buff, &PassFlag, len);
            }
        }
    }
...
int loopback_read( char *page, char **start, off_t off, int count, int *eof, void *data )
{
    ...
    len = snprintf(page, PAGE_SIZE, "current status %d\nnot 0 : test loop, 0 : not test yet or test finish\n", (unsigned int)test_num);
...
int init_lan_proc( void )
{
    ...
    snprintf(name, MAX_NAMELEN, "%s", "loopback");

    eth_dir = proc_mkdir("eth", NULL);

    proc_entry = create_proc_entry( name, 0644, eth_dir );
    if (proc_entry == NULL) {
        ret = -ENOMEM;
        printk("eth: Couldn't create proc entry\n");
    } else {
        proc_entry->read_proc = loopback_read;
        proc_entry->write_proc = loopback_write;
        printk("eth: Module loaded.\n");
    }
...
static int MDev_EMAC_get_info(struct net_device *dev)
{
    ...
    MHal_EMAC_read_phy (phyaddr, MII_BMSR, &bmsr);
    MHal_EMAC_read_phy (phyaddr, MII_BMSR, &bmsr);
    if (!(bmsr & (u32)BMSR_LSTATUS)){
        uRegStatus &= ~((u32)ETHERNET_TEST_RESET_STATE);
    ...
    if (bmcr & BMCR_ANENABLE)
    {
        ...
        {
            uRegStatus &= ~((u32)ETHERNET_TEST_SPEED_100M); //SPEED_10//
        }
        ...
        {
            uRegStatus &= ~((u32)ETHERNET_TEST_DUPLEX_FULL); //DUPLEX_HALF//
        }
    }
    else
    {
...
{
    u32 value;
    value = (u32)((dev->dev_addr[3] << 24) | (dev->dev_addr[2] << 16) | (dev->dev_addr[1] << 8) |(dev->dev_addr[0]));
    MHal_EMAC_Write_SA1L(value);
    value = (u32)((dev->dev_addr[5] << 8) | (dev->dev_addr[4]));
...
static void MDev_EMAC_get_mac_address (struct net_device *dev)
{
    ...
    addr[0] = (char)((LoAddr & 0xff));
    addr[1] = (char)((LoAddr & 0xff00) >> 8);
    addr[2] = (char)((LoAddr & 0xff0000) >> 16);
    addr[3] = (char)((LoAddr & 0xff000000) >> 24);
    addr[4] = (char)((HiAddr & 0xff));
    addr[5] = (char)((HiAddr & 0xff00) >> 8);
    ...
    // use default value for mac address
	// EMAC use FF:FF:FF:FF:FF:FF for default. request by sec
    memcpy(dev->dev_addr, &MY_MAC, 6);    
}
...
// allowing EMAC use FF:FF:FF:FF:FF:FF. request by sec
#if 0
...
static void MDev_EMAC_set_rx_mode (struct net_device *dev)
{
    ...
    else if (dev->flags & (unsigned int)(~IFF_PROMISC))
    {   // Disable promiscuous mode //
        uRegVal &= ~((unsigned int)EMAC_CAF);
    }
    MHal_EMAC_Write_CFG(uRegVal);

    if (dev->flags & IFF_ALLMULTI)
    {   // Enable all multicast mode //
        MHal_EMAC_update_HSH(0xFFFFFFFF, 0xFFFFFFFF);
        uRegVal |= EMAC_MTI;
    }
    ...
    else if (dev->flags & (unsigned int)(~(IFF_ALLMULTI | IFF_MULTICAST)))
    {   // Disable all multicast mode//
        MHal_EMAC_update_HSH(0,0);
        uRegVal &= ~((unsigned int)EMAC_MTI);
    }
...
{
    u32 value;
    MHal_EMAC_read_phy ((unsigned char)phy_id, (unsigned char)location, &value);
    return (int)value;
}

static void MDev_EMAC_mdio_write (struct net_device *dev, int phy_id, int location, int value)
{
    MHal_EMAC_write_phy ((unsigned char)phy_id, (unsigned char)location, (unsigned int)value);
}
...
static int MDev_EMAC_ethtool_ioctl (struct net_device *dev, void *useraddr)
{
    ...
    switch (ethcmd)
    {
        ...
        case ETHTOOL_GLINK:
        {
            ...
            edata.data = (u32)mii_link_ok (&LocPtr->mii);
...
static int MDev_EMAC_ioctl (struct net_device *dev, struct ifreq *rq, int cmd)
{
    ...
    switch (cmd)
    {
        ...
        case SIOCDEVPRIVATE:
			rq->ifr_metric = (int)(MDev_EMAC_get_info(emac_dev) |(int)initstate);
		    return 0;
        ...
        case SIOCGMIIREG:
            ...
            if((data->reg_num & 0x1f) == 0x1) 
            {
                ...
                spin_lock_irqsave(LocPtr->lock, flags);
                data->val_out = (u16)phy_status_register;
                spin_unlock_irqrestore(LocPtr->lock, flags);
            }
            else 
            {
                MHal_EMAC_read_phy((phyaddr & 0x1F), (data->reg_num & 0x1f), &temp_data);
                data->val_out = (u16)temp_data;
            }
            return 0;
...
static int MDev_EMAC_close (struct net_device *dev)
{
    ...
    ThisBCE.connected = 0;
    LocPtr->ep_flag &= (~((unsigned long)EP_FLAG_OPEND));
...
static int MDev_EMAC_TxReset(void)
{
    ...
    MHal_EMAC_Write_CTL((val & ~((u32)EMAC_TE)));
...
static dma_addr_t get_tx_addr(void)
{
    ...
    addr = (dma_addr_t)(EMAC_TX_PTK_BASE + 2048*(unsigned int)txidx);
    txidx ++;
...
static int MDev_EMAC_BGsend(struct net_device* dev, u32 addr, int len )
{
    ...
    memcpy((void*)skb_addr,(void *)addr, (__kernel_size_t)len);

    LocPtr->stats.tx_bytes += (unsigned long)len;

#ifdef CHIP_FLUSH_READ
    #if defined(CONFIG_MIPS)
    if((unsigned int)skb_addr < 0xC0000000)
    {
        Chip_Flush_Memory_Range((unsigned int)skb_addr&0x0FFFFFFF, (unsigned long)len);
    }
    else
    {
...
static void MDev_EMAC_Send_PausePkt(struct net_device* dev)
{
    ...
    //Disable Rx
    MHal_EMAC_Write_CTL((val & ~((u32)EMAC_RE)));
...
static int MDev_EMAC_rx (struct net_device *dev)
{
    ...
    #else
        if(loop_back_test && (--test_num >= 0))
        {
            u32 bytes;
            for(bytes = 0; bytes < sizeof(TCP_packet); bytes++)
            {
                if(*(rx_skb[LocPtr->rxBuffIndex]->data + bytes) != *(test_packet->data + bytes))
                {
                    printk("loop back test failed!! received packet is not the same to transmitted packet\n");
                    test_num = -1;
                    break;
                }
            }
...
static int MDev_EMAC_rx (struct net_device *dev)
{
    ...
    do
    {
        ...
        p_recv = (char *) ((((dlist->descriptors[LocPtr->rxBuffIndex].addr) & 0xFFFFFFFF) + RAM_VA_PA_OFFSET + MIU0_BUS_BASE) &~((u32)(EMAC_DESC_DONE | EMAC_DESC_WRAP)));
        pktlen = dlist->descriptors[LocPtr->rxBuffIndex].size & 0x7ff;    /* Length of frame including FCS */
        ...
        if (skb != NULL)
        {
            ...
            if(loop_back_test && (--test_num >= 0))
            {
                ...
                for(bytes = 0; bytes < sizeof(TCP_packet); bytes++)
                {
                    if(*(p_recv + bytes) != *(test_packet->data + bytes))
                    {
                        printk("loop back test failed!! received packet is not the same to transmitted packet\n");
                        test_num = -1;
                        break;
        ...
        dlist->descriptors[LocPtr->rxBuffIndex].addr  &= ~((unsigned int)EMAC_DESC_DONE);  /* reset ownership bit */
    #ifdef CHIP_FLUSH_READ
    #if defined(CONFIG_MIPS)
        Chip_Flush_Memory_Range((unsigned int)(&(dlist->descriptors[LocPtr->rxBuffIndex].addr)) & 0x0FFFFFFF, sizeof(dlist->descriptors[LocPtr->rxBuffIndex].addr));
    #elif defined(CONFIG_ARM)
...
static int MDev_EMAC_loopback_test(void)
{
    ...
    if(ret == -1){
        printk("--------PHY Disconnect happen--------\n");
	loop_back_test = 0;
	return ret;
    }

    test_packet = alloc_skb(SOFTWARE_DESC_LEN, GFP_ATOMIC);
    packet_physaddr = get_tx_addr();
    memcpy((void *)packet_physaddr, (void *)vAddr, sizeof(TCP_packet));
    memcpy(test_packet->data, (void *)vAddr, sizeof(TCP_packet));
    ...
    MHal_EMAC_Write_TAR(packet_physaddr - RAM_VA_PA_OFFSET - MIU0_BUS_BASE);

    // start transmit
    MHal_EMAC_Write_TCR(sizeof(TCP_packet));
...
static u32 MDev_EMAC_VarInit(void)
{
    ...
    EMAC_DBG("alloRAM_PA_BASE= 0x%X  alloRAM_SIZE= 0x%X\n", alloRAM_PA_BASE, alloRAM_SIZE);
    EMAC_DBG("alloRAM_VA_BASE= 0x%X  alloRAM_SIZE= 0x%X\n", (unsigned int)alloRAM_VA_BASE, alloRAM_SIZE);
    //EMAC_DBG("high_memory:0x%x\n", high_memory);
    ...
    RAM_VA_BASE       = ((u32)alloRAM_VA_BASE + sizeof(struct EMAC_private) + 0x3FFF) & ~((u32)0x3FFF);   // IMPORTANT: Let lowest 14 bits as zero.
    RAM_PA_BASE       = ((u32)alloRAM_PA_BASE + sizeof(struct EMAC_private) + 0x3FFF) & ~((u32)0x3FFF);   // IMPORTANT: Let lowest 14 bits as zero.
    ...
    //EMAC_DBG("RAM_VA_BASE:%x\n", RAM_VA_BASE);
    //EMAC_DBG("RAM_PA_BASE:%x\n", RAM_PA_BASE);
    //EMAC_DBG("RX_BUFFER_BASE:%x\n", RX_BUFFER_BASE);
    //EMAC_DBG("RBQP_BASE:%x\n", RBQP_BASE);
    //EMAC_DBG("TX_BUFFER_BASE:%x\n", TX_BUFFER_BASE);
    //EMAC_DBG("RAM_VA_PA_OFFSET:%x\n", RAM_VA_PA_OFFSET);
    //EMAC_DBG("TX_SKB_BASE:%x\n", TX_SKB_BASE);
...
static int MDev_EMAC_setup (struct net_device *dev, unsigned long phy_type)
{
    ...
    dev->base_addr = (unsigned long) REG_ADDR_BASE;
    MDev_EMAC_HW_init();
    dev->irq = E_IRQ_EMAC;
...
static int MDev_EMAC_SwReset(struct net_device *dev)
{
    ...
    oldCTL = MHal_EMAC_Read_CTL() & ~((u32)(EMAC_TE | EMAC_RE));
...
static void MDev_EMAC_timer_callback(unsigned long value)
{
    ...
    if(ret == -1 && ThisBCE.connected == 1){
        printk("--------PHY Disconnect happen--------\n");
        printk("- KDISCONN: %d=,%d\n",ret,ThisBCE.connected);
        printk("-------------------------------------\n");
    }
    else if(ret == 0 && ThisBCE.connected == 0){
        printk("----------PHY Connect again----------\n");
        printk("- KCONNECT: %d=,%d\n",ret,ThisBCE.connected);
        printk("-------------------------------------\n");
    }
...
{
    remove_proc_entry(name, eth_dir);
