...
static int stl_do_ioctl(u32 volume, u32 partno, u32 cmd, u32 arg)
{
	...
	switch (cmd) 
	{
		...
		case FSR_GET_PART_INFO:
		{
			...
			info.page_ssize = vs->nSparePerSct  * vs->nSctsPerPg;
							...
							unsigned cmd, unsigned long arg)
{
	...
	switch (cmd)
	{
		...
		case STL_DISCARD:
		{
			u32 discard_bsize;
			dev_t discard_dev;
			struct data_region discard_data;
			DEBUG(DL3, "IN STL_DISCARD : 0x%x", cmd);
			ret = copy_from_user((char *)&discard_data, (char *)arg, sizeof(struct data_region));
			/* memory error */
			if (ret)
			{
				ERRPRINTK("copy_from_user error [0x%08x]", ret);
				ret = -EFAULT;
				return ret;
			}

#if LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 28)
			discard_dev = bdev->bd_dev;
			discard_bsize = bdev->bd_block_size;
#else
			discard_dev = inode->i_rdev;
			discard_bsize = 1 << inode->i_blkbits;
#endif
			ERRPRINTK("Entering the discard request\n");
			ret = sec_stl_delete(discard_dev, discard_data.start, discard_data.num, discard_bsize);
			if (ret == 0)
				ERRPRINTK("stl_delete is sucessfull\n"); 
			if (ret)
			{
				ERRPRINTK("Fail stl_delete (start: %u, num=%u blocksize=%u\n",
						discard_data.start, discard_data.num, discard_bsize);
				return ret;
			}

			DEBUG(DL3, "OUT STL_DISCARD : 0x%x", cmd);
			return 0;
		}
