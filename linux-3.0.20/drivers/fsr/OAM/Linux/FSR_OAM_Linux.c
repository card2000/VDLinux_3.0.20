...
extern  int     semaphore_use_bml[FSR_MAX_VOLS];
extern  int     semaphore_fsr_bml_handle[FSR_MAX_VOLS];

#else

struct  semaphore fsr_sem[FSR_OAM_MAX_SEMAPHORES];
int     semaphore_use[FSR_OAM_MAX_SEMAPHORES] = {0,};
int     semaphore_use_bml[FSR_MAX_VOLS] = {0,};
int     semaphore_fsr_bml_handle[FSR_MAX_VOLS] = {0,};
                 ...
                 UINT32  nLayer)
{
    ...
    do
    {
	   ...
	   || (nLayer == FSR_OAM_SM_TYPE_BML_VOL_1))
        {
            if (semaphore_use_bml[LAYER2IDX(nLayer)]++ > 0)
            {
                *pHandle = semaphore_fsr_bml_handle[LAYER2IDX(nLayer)];
                break;
            }
        }
	   ...
	   || (nLayer == FSR_OAM_SM_TYPE_BML_VOL_1))
        {
            semaphore_fsr_bml_handle[LAYER2IDX(nLayer)] = sem_count;
        }
