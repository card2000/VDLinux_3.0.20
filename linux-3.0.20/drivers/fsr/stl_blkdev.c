...
static DEFINE_SEMAPHORE(stl_list_mutex);
#else 
...
static void stl_request(struct request_queue *rq)
{
...
#endif
	{
		...
		DEBUG(DL3,"STL[O]: Before entering stl delete function \n");

		/* The discard function is available from 2.6.28 onwards */
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 28)
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 31)

		DEBUG(DL3,"STL[O]: Before entering stl delete function in the  first loop\n");
		DEBUG(DL3,"STL[O]: req value (%d) \n", req->cmd_flags & REQ_DISCARD);
		
		if (req->cmd_flags & REQ_DISCARD) {
			spin_unlock_irq(rq->queue_lock);
		ret =  sec_stl_delete(MKDEV(dev->gd->major, minor), blk_rq_pos(req), blk_rq_sectors(req), SECTOR_SIZE);

		DEBUG(DL3,"STL[O]: sec_stl_delete return value(%d)\n", ret);
			if (ret) {
				ERRPRINTK("Fail stl_delete (minor=%u start: %llu, num=%u blocksize=%u\n",
						minor, (long long)(blk_rq_pos(req)), blk_rq_sectors(req), SECTOR_SIZE);
			}
			spin_lock_irq(rq->queue_lock);
			__blk_end_request_all(req, ret);
#else

		DEBUG(DL3,"STL[O]: Before entering stl delete function in the second loop\n");
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,38) 
	dev->queue->flush_flags = 0;
#else 
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 28)
#if LINUX_VERSION_CODE <= KERNEL_VERSION(2, 6, 35)
	blk_queue_ordered(dev->queue, QUEUE_ORDERED_DRAIN, NULL);
#else
	blk_queue_ordered(dev->queue, QUEUE_ORDERED_DRAIN);
#endif /* LINUX 2.6.35 */
#endif /* LINUX 2.6.28 */
#endif /* LINUX 2.6.38 */
...
#ifdef MULTI_VOLUME
	snprintf(dev->gd->disk_name, 32, "%s%d/%d", DEVICE_NAME,
				minor>>PARTITION_BITS, minor%(1<<PARTITION_BITS));
#else
