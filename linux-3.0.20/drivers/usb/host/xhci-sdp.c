...
static int sdp_hcd_probe(struct hc_driver *driver, struct platform_device *pdev)
{
	...
	if (!sdp_revision_id) {
		reg_addr = __arm_ioremap(mem_start, mem_len, MT_DEVICE_WC);
		dev_info(&pdev->dev, "call __arm_ioremap, ES0 chip\n");
	} else {
		reg_addr = ioremap_nocache(mem_start, mem_len);
		dev_info(&pdev->dev, "call ioremap_nocache, ES1 chip\n");
	}
...
#ifdef CONFIG_USB_XHCI_SDP_WORKAROUND
	dev_info(&pdev->dev, "initializing SDP-SoC USB3 W Controller\n");
#else
