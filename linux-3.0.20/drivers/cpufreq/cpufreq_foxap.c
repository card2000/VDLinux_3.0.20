/*
 *  drivers/cpufreq/cpufreq_foxap.c
 *
...
#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_FOXAP
static
#endif
struct cpufreq_governor cpufreq_gov_foxap = {
       .name                   = "foxap",
...
static void foxap_powersave_bias_init_cpu(int cpu)
{
...
static void foxap_powersave_bias_init(void)
{
	int i;
	for_each_online_cpu(i) {
		foxap_powersave_bias_init_cpu(i);
	}
}
...
static const int log_buf_size = SZ_4K;
...
/* cpufreq_foxap Governor Tunables */
...
static void dbs_freq_increase(struct cpufreq_policy *p, unsigned int freq)
{
	...
	if (0 > __cpufreq_driver_target(p, freq, dbs_tuners_ins.powersave_bias ?
...
static void dbs_check_cpu(struct cpu_dbs_info_s *this_dbs_info)
{
	...
	for_each_cpu(j, policy->cpus) {
		...
		/*
		 * For the purpose of foxap, waiting for disk IO is an
	 ...
	 */
	if (max_load_freq < dbs_tuners_ins.down_threshold * policy->cur) {
		unsigned int freq_next;
		freq_next = max_load_freq / dbs_tuners_ins.down_threshold;
		...
		dvfs_print("decrease freq(%d)\n", freq_next);

		if (!dbs_tuners_ins.powersave_bias) {
			if (0 > __cpufreq_driver_target(policy, freq_next,
						CPUFREQ_RELATION_L))
				policy->cur = cpufreq_get(0);
		} else {
			int freq = powersave_bias_target(policy, freq_next,
					CPUFREQ_RELATION_L);
			if (0 > __cpufreq_driver_target(policy, freq,
					CPUFREQ_RELATION_L))
				policy->cur = cpufreq_get(0);
		}
	}
				   ...
				   unsigned int event)
{
	...
	case CPUFREQ_GOV_START:
		...
		this_dbs_info->rate_mult = 1;
		foxap_powersave_bias_init_cpu(cpu);
		/*
...
static int __init cpufreq_gov_dbs_init(void)
{
	...
	put_cpu();
	if (idle_time != -1ULL) {
		printk("cpufreq - micro accounting support\n");
		...
		*/
		...
		printk("cpufreq - 10 tick * %d sampling support\n", MIN_SAMPLING_RATE_RATIO);
	...
	return cpufreq_register_governor(&cpufreq_gov_foxap);
}
...
static void __exit cpufreq_gov_dbs_exit(void)
{
	...
	cpufreq_unregister_governor(&cpufreq_gov_foxap);
}

#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_FOXAP
