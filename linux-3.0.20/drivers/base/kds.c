		...
		unsigned long         jiffies_timeout)
{
	...
	{
		long wait_res = 0;
		long timeout = jiffies_timeout == KDS_WAIT_BLOCKING ? MAX_SCHEDULE_TIMEOUT : jiffies_timeout;

		if (timeout)
				wait_res = wait_event_interruptible_timeout(wake, rset->pending == 0, timeout);
