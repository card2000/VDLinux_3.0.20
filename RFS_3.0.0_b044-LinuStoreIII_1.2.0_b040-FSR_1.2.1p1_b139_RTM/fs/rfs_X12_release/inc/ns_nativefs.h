...
/**
* @brief SPINLOCK struct
* @ref _SPINLOCK
*/
typedef struct _SPINLOCK					NS_SPINLOCK, *NS_PSPINLOCK;
...
/**
 * @brief This function pointer is a declaration of the "pfnClearNode" defined by native filesystem
 * @param[in]   pVnode              NS_PVNODE
 * @return      FERROR
 * @exception   FERROR_NO_ERROR         success
 * @par Implementation : Optional
 * @li  Called to tell native filesystem that the node is no longer useful
 * @li  If native has local resource on vnode, implements this
 * @par To do :
 * @li  Release native buffers for native vnode
 */
typedef FERROR (*PFN_NS_VNODE_OPS_CLEAR_NODE) (
	IN  NS_PVNODE       pVnode);
...
#define NS_CreateSpin					RtlCreateSpinlock
#define NS_LockSpin						RtlGetSpinlock
#define NS_UnlockSpin					RtlPutSpinlock
#define NS_DestroySpin					RtlDestroySpinlock
...
/** @brief NS_CreateSpinlock function creates and returns a semaphore object. */
unsigned int			
NS_CreateSpin(
	IN NS_SPINLOCK*	pSpinlock);

/** @brief NS_DestroySpinlock function destroys the semaphore and frees it from the
 * system. */
void
NS_DestroySpin(
	IN NS_SPINLOCK*	pSpinlock);

/** @brief NS_GetSpinlock function gets an exclusive rights to the semaphore. */
unsigned int
NS_LockSpin(
	IN NS_SPINLOCK*	pSpinlock);


/** @brief NS_PutSpinlock function releases the semaphore. */
unsigned int
NS_UnlockSpin(
	IN NS_SPINLOCK*	pSpinlock);
