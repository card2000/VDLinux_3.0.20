...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 38)
typedef const struct inode		LINUX_INODE_CONST, *PLINUX_INODE_CONST;
typedef const struct dentry		LINUX_DENTRY_CONST, *PLINUX_DENTRY_CONST;
typedef const struct qstr		LINUX_QSTR_CONST, *PLINUX_QSTR_CONST;
#endif
...
typedef struct writeback_control               *PLINUX_WRITEBACK;
#endif
...
#define LINUX_GFP_NOFS		GFP_NOFS
...
#define LINUX_MountFs			mount_bdev
#endif
...
#define LINUX_InodeSetattr              setattr_copy
#else
...
#define LINUX_Vmtruncate	vmtruncate
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 39)
extern struct dentry *LxMount(struct file_system_type *plxFsType, int dwFlag, const char* pDevName, void* pData);
#endif
